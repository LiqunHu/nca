...
/* wj */
call Pro_AddMenu('长期资产管理', '资产盘点管理', '/nca/longtermassets/NCAAssetInventoryControl', 'NCAASSETINVENTORYCONTROL');
...
update tbl_nca_taskallot set taskallot_name ='资金支出管理批任务',taskallot_describe = '分配处理资金支出管理审核人员' where taskallot_name like '%特殊费用报销审批任务%';
update tbl_common_domainmenu set domainmenu_name='资金支出管理' where domainmenu_name like '%特殊费用报销%';

ALTER TABLE `tbl_nca_specialexpense` ADD COLUMN `s_capital_cost_type` INTEGER AFTER `s_expense_description`;
ALTER TABLE `tbl_nca_corporateclients` ADD COLUMN `creditline_money` DOUBLE DEFAULT 0 AFTER `settlement_way`;
ALTER TABLE `tbl_nca_corporateclients` ADD COLUMN `creditline_use` DOUBLE DEFAULT 0 AFTER `creditline_money`;
ALTER TABLE `tbl_nca_corporateclients` ADD COLUMN `creditline_advance` DOUBLE DEFAULT 0 AFTER `creditline_use`;
ALTER TABLE `tbl_nca_order` ADD COLUMN `purchaser_corporateclients_id` INTEGER AFTER `sap_order_state`;
ALTER TABLE `tbl_nca_order` ADD COLUMN `send_creditline_state` INTEGER DEFAULT 0 AFTER `purchaser_corporateclients_id`;
...
call Pro_AddMenu('出纳管理', '收款申报', '/nca/cashiermanage/NCAGatheringControl','NCAGATHERINGCONTROL');
call Pro_AddMenu('出纳管理', '付款确认', '/nca/cashiermanage/NCAPaymentConfirmControl','NCAPAYMENTCONFIRMCONTROL');
...
DROP TABLE IF EXISTS `tbl_nca_cashiergathering`;
CREATE TABLE `tbl_nca_cashiergathering` (
...
DROP TABLE IF EXISTS `tbl_nca_paymentconfirm`;
CREATE TABLE `tbl_nca_paymentconfirm` (
...
DROP TABLE IF EXISTS `tbl_nca_creditlinedetail`;
CREATE TABLE `tbl_nca_creditlinedetail` (
