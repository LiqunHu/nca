...
/* ty */
ALTER TABLE tbl_nca_corporateclients MODIFY COLUMN month_settlement datetime;

ALTER TABLE `tbl_nca_corporateclients`
ADD COLUMN `corporateclients_mobile_phone` varchar(100) AFTER `corporateclients_address`;
/*end ty */

/* ls */

ALTER TABLE `tbl_nca_customer`
ADD COLUMN `customer_point` int(11)  DEFAULT 0 AFTER `customer_reimburserank_id`;


DROP TABLE IF EXISTS `tbl_nca_pointtype`;
CREATE TABLE `tbl_nca_pointtype` (
...
DROP TABLE IF EXISTS `tbl_nca_customerpoint`;
CREATE TABLE `tbl_nca_customerpoint` (
...
INSERT INTO tbl_nca_pointtype(pointtype_id,customerpoint_name,base_point,pointtype_remarks,created_at,updated_at)
VALUES(1,'注册积分',100,'用户首次注册送积分',now(),now());

update tbl_nca_customer set customer_point=100;
