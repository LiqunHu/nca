...
/* ty */
update tbl_common_api set api_name = '报废申请单列表' where api_function = 'NCAASSETRETIREMENTCONTROL';
update tbl_common_domainmenu set domainmenu_name = '报废申请单列表' where api_function = 'NCAASSETRETIREMENTCONTROL';
update tbl_common_systemmenu set systemmenu_name = '报废申请单列表' where api_function = 'NCAASSETRETIREMENTCONTROL';
update tbl_common_templatemenu set templatemenu_name = '报废申请单列表' where api_function = 'NCAASSETRETIREMENTCONTROL';

call Pro_AddMenu('长期资产管理', '资产报废管理','/nca/longtermassets/NCAAssetRetirementDetailControl','NCAASSETRETIREMENTDETAILCONTROL');

update tbl_nca_taskallot set taskallot_name = '出差、用车接待申请通知' where taskallot_id = '22';
/*end ty */

/* njc */

call Pro_AddMenu('长期资产管理', '材料申购单','/nca/longtermassets/NCAAmortizeScribeOrderControl','NCAAMORTIZESCRIBEORDERCONTROL');
call Pro_AddMenu('长期资产管理', '材料采购单','/nca/longtermassets/NCAAmortizePurchaseOrderControl','NCAAMORTIZEPURCHASEORDERCONTROL');
insert into seqmysql values ('subcribeOrderIDSeq',1,1,99999999);
insert into seqmysql values ('amoPurOrderIDSeq',1,1,99999999);

ALTER TABLE tbl_nca_amortizesubscribe ADD COLUMN subscribe_price double NUll DEFAULT 0 AFTER `subscribe_number`;
ALTER TABLE tbl_nca_amortizesubscribe ADD COLUMN suppler_id varchar(30) NUll DEFAULT '' AFTER `subscribe_price`;
ALTER TABLE tbl_nca_amortizesubscribe change suppler_id  supplier_id VARCHAR(30);

DROP TABLE IF EXISTS `tbl_nca_amortizesubscribeorder`;
CREATE TABLE `tbl_nca_amortizesubscribeorder` (
...
DROP TABLE IF EXISTS `tbl_nca_amortizesubscribeorderdetail`;
CREATE TABLE `tbl_nca_amortizesubscribeorderdetail` (
...
DROP TABLE IF EXISTS `tbl_nca_amortizesubscribeorderdetailend`;
CREATE TABLE `tbl_nca_amortizesubscribeorderdetailend` (
...
DROP TABLE IF EXISTS `tbl_nca_amortizepurchaseorder`;
CREATE TABLE `tbl_nca_amortizepurchaseorder` (
...
DROP TABLE IF EXISTS `tbl_nca_amortizepurchaseorderdetail`;
CREATE TABLE `tbl_nca_amortizepurchaseorderdetail` (
