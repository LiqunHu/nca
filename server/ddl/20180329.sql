...
call Pro_AddMenu('运营数据管理', '工人查询', '/nca/baseconfig/NCAWorkerBranchControl', 'NCAWORKERBRANCHCONTROL');

/*end ls */

/* LJZ */
DROP TABLE IF EXISTS `tbl_nca_businessdomain`;
CREATE TABLE `tbl_nca_businessdomain` (
...
DROP TABLE IF EXISTS `tbl_nca_produce_client`;
CREATE TABLE `tbl_nca_produce_client` (
...
ALTER TABLE `tbl_nca_produce` ADD COLUMN `produce_client_state` varchar(5) DEFAULT NULL;
ALTER TABLE `tbl_nca_produce` ADD COLUMN `start_date`  datetime;
ALTER TABLE `tbl_nca_produce` ADD COLUMN `end_date` datetime;
