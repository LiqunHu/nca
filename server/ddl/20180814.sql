...
/* njc */
call Pro_AddMenu('生产计划管理', '生产任务单','/nca/productionmanage/NCAProductiveTaskControl','NCAPRODUCTIVETASKCONTROL');

alter table tbl_nca_orderproductplan modify column order_id varchar(30);

DROP TABLE IF EXISTS `tbl_nca_productivetask`;
CREATE TABLE `tbl_nca_productivetask` (
...
DROP TABLE IF EXISTS `tbl_nca_delivery`;
CREATE TABLE `tbl_nca_delivery` (
...
DROP TABLE IF EXISTS `tbl_nca_deliveryitem`;
CREATE TABLE `tbl_nca_deliveryitem` (
...
call Pro_AddMenu('订单管理', '订单评审', '/nca/ordermanage/NCAOrderReviewControl', 'NCAORDERREVIEWCONTROL');
