...
const tb_inquiry = model.nca_inquiry;
...
async function initAct(req, res) {
    try {
        ...
        let templates = await sequelize.query('select a.produce_id as id, a.produce_id as value, b.materiel_name as text from tbl_nca_produce a left join tbl_nca_materiel b on (a.materiel_id=b.materiel_id) where b.state = 1 and a.state = 1 and b.materiel_type = 10', {
            replacements:[],
...
async function quoteAct(req, res) {
    try {
        ...
        let template = await sequelize.query('select b.materiel_sale from tbl_nca_produce a left join tbl_nca_materiel b on (a.materiel_id=b.materiel_id) where a.produce_id = ?', {
