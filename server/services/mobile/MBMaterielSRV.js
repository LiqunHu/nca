...
let searchAct = async(req, res) => {
    try {
        ...
        for (let r of rooms) {
            ...
            let queryStr = `select m.*,om.*,ic.total_count from tbl_nca_ordermateriel om
            left join tbl_nca_materiel m 
            on m.materiel_id = om.materiel_id
            left join (select sum(account_operate_amount) as total_count,materiel_id from tbl_nca_inventoryaccount where order_id=? and account_operate_type = 1 group by materiel_id) as ic
