...
const NCACmsSRV = require('../nca/cms/NCACmsSRV');
const CryptoJS = require('crypto-js');
const crypto = require('crypto');
const point = require('../nca/baseconfig/NCAPointControlSRV');





const tb_decorateinfo = model.nca_decorateinfo;
const tb_uploadfile = model.nca_uploadfile;
const tb_usercollection = model.nca_usercollection;
        ...
        for (let d of decorates) {
            let files = await tb_uploadfile.findAll({
                where: {
                    ...
                    api_name: 'NCASMALLPROGRAMCONTROL',
                    state: GLBConfig.ENABLE
                }
            });
            //nca用户1 微信用户2
...
async function getCollectionAct(req, res) {
    try {
        ...
        for (let c of collection) {
            ...
            let packages =  await NCACmsSRV.getCmsContentById(c.decorate_id)
...
async function addCollectionAct(req, res) {
    try {
        let doc = common.docTrim(req.body)//nca用户传1 微信用户2
        let addrow = null;
        if (doc.type === '1') {
