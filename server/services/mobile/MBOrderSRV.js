...
let searchAct = async(req, res) => {
    try {
        let doc = common.docTrim(req.body),
            user = req.user;
        let queryStr = ` select o.*,u.*,t.template_name from tbl_nca_order o
        left join tbl_common_user u on o.user_id = u.user_id
        left join tbl_nca_template t on t.template_id = o.template_id
        where  o.state = ? `;
...
let detailAct = async(req, res) => {
    try {
        let doc = common.docTrim(req.body),
            user = req.user,
            returnData = {};
        let queryStr = ` select o.*,u.*, m.materiel_name, e.estate_name, r.build, r.unit, r.room_no from tbl_nca_order o
        left join tbl_nca_produce t on t.produce_id = o.produce_id
        left join tbl_nca_materiel m on t.materiel_id = m.materiel_id
        left join tbl_common_user u on o.user_id = u.user_id
