...
const TaskListControlSRV = require('../nca/baseconfig/NCATaskListControlSRV');
const PointControlSRV = require('../nca/baseconfig/NCAPointControlSRV.js');
...
const tb_comment = model.nca_comment;
...
const tb_room = model.nca_orderroom;
...
let searchAct = async(req, res) => {
    try {
        let doc = common.docTrim(req.body),
            user = req.user,
            returnData = {};
        let queryStr = ` select g.*,o.order_id,u.name as worker_name,u.avatar as worker_avatar, o.order_address from tbl_nca_gantttasks g
        left join tbl_common_user u on g.leader_id = u.user_id left join tbl_nca_order o
        ...
        let date = new Date();
        for (let t of tasks) {
            let countStr = `select DISTINCT(room_id) from tbl_nca_orderacceptance where order_id = ? and gantttasks_id = ?`
            ...
            countStr = `select count(1) acount from tbl_nca_orderacceptance where order_id = ? and gantttasks_id = ?`
...
let taskDetailAct = async(req, res) => {
    try {
        ...
        from tbl_nca_orderacceptance a, tbl_nca_orderroom b
...
let acceptanceDetailAct = async(req, res) => {
    try {
        ...
        if (order.order_type === '7') {
            apiName = 'NCAROOMTYPEDETAILCONTROL'
        } else {
            apiName = 'NCAPRODUCECONTROL'
        }
...
let acceptanceSearchAct = async(req, res) => {
    try {
        ...
        let roomStr = `SELECT a.*, b.text, e.estate_name, r.build, r.unit, r.room_no, s.room_name
                        FROM tbl_nca_orderacceptance a
                        left join tbl_nca_gantttasks b on a.gantttasks_id = b.gantttasks_id
                        left join tbl_nca_order o      on a.order_id = o.order_id
                        left join tbl_nca_estate e     on o.estate_id = e.estate_id
                        left join tbl_nca_estateroom r on o.estate_room_id = r.room_id
                        left join tbl_nca_orderroom s  on a.room_id = s.room_id
