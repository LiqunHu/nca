...
const tb_feedback = model.nca_feedback;
...
let searchAct = async(req, res)=> {
    try {
        ...
        let queryStr = ` select f.*,u.username,u.name from tbl_nca_feedback f left join tbl_common_user u on f.creator = u.user_id 
          where  f.state = ? `;
