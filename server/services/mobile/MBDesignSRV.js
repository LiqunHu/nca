...
const tb_upload_file = model.nca_uploadfile;
...
let searchAct = async(req, res) => {
    try {
        ...
        //个单获取设计文件
            let queryStr = `select uf.* from tbl_nca_orderdesign od 
            left join tbl_nca_uploadfile uf 
            on od.order_id = uf.order_id and od.require_type = uf.srv_type and od.design_id = uf.srv_id
            left join tbl_nca_orderrequire ore
            on od.require_id = ore.require_id
            where od.order_id = ? and uf.state = 1 and uf.api_name = 'NCAORDERDETAILCONTROL' and uf.srv_type = 1 and ore.require_hidden = '1' `
