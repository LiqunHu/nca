...
const tb_specialexpense = model.nca_specialexpense;
...
exports.NCASpecialExpenseSRV = (req, res) => {
...
let initAct = async (req, res) => {
    try {
        ...
        logger.error('NCASpecialeExpenseSRV-initAct:' + error);
...
let searchAct = async (req, res) => {
    try {
        ...
        let query = 'select b.*,p.paymentconfirm_state from tbl_nca_specialexpense b   ' +
            'left join tbl_common_domain a on a.domain_id = b.domain_id ' +
            'left join tbl_nca_paymentconfirm p on (b.s_expense_code = p.paymentconfirm_source_code and p.state=1)' +
        ...
        logger.error('NCASpecialeExpenseSRV-searchAct:' + error);
        common.sendFault(res, error);
    }
}
...
let addAct = async (req, res) => {
    try {
        ...
        logger.error('NCASpecialeExpenseSRV-addAct:' + error);
        common.sendFault(res, error);
    }
}
...
let removeAct = async (req, res) => {
    try {
        ...
        logger.error('NCASpecialeExpenseSRV-removeAct:' + error);
        common.sendFault(res, error);
    }
};
