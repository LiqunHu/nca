...
const tb_reimburserank = model.nca_reimburserank;
...
exports.NCATransReceptionDetailResource = (req, res) => {
...
let initAct = async (req, res) => {
    try {
        ...
        logger.error('NCATransReceptionResource-initAct:' + error);
...
let searchAct = async (req, res) => {
    try {
        ...
        let queryStr = 'select a.*,b.customer_reimburserank_id,r.reimburserank_id,r.reimburserank_name,' +
            ...
            ' from tbl_nca_transreceptionapply a ' +
            'left join tbl_nca_customer b on b.user_id = a.trapply_creator_id ' +
            'left join tbl_common_user d on d.user_id = a.trapply_confirm_id ' +
            'left join tbl_nca_reimburserank r on r.reimburserank_id = b.customer_reimburserank_id ' +
        ...
        logger.error('NCATransReceptionResource-searchAct:' + error);
        common.sendFault(res, error);
    }
};
...
let modifyAct = async (req, res) => {
    try {
        ...
        logger.error('NCATransReceptionResource-modifyAct:' + error);
        common.sendFault(res, error);
    }
}
