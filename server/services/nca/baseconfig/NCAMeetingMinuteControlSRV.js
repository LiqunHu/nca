...
const logger = require('../../../util/Logger').createLogger('NCAMeetingMinuteControlSRV');
...
const tb_meetingfollow = model.nca_meetingfollow;
const tb_task = model.nca_task;
const tb_uploadfile = model.nca_uploadfile;

//行政办公管理->会议记录列表
exports.NCAMeetingMinuteControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr='select t.*,rt.meetingroom_name,ut.`name` as host_name,ut2.`name` as meeting_remark_username,ut3.`name` as meeting_decision_username from tbl_nca_meeting t ' +
...
async function initEquipmentAct(req, res) {
    try {
        ...
        let queryStr = 'select t.meetingroomequipment_id as id,t.equipment_name as text from tbl_nca_meetingroomequipment t where t.meetingroom_id=?';
...
async function searchFollowAct(req, res) {
    try {
        ...
        let queryStr='select t.* , ut.name as executor_name,ut2.name as checker_id_name from tbl_nca_meetingfollow t ' +
...
async function searchFilesAct(req, res) {
    try {
        ...
        let queryStr = `select * from tbl_nca_meeting t where t.state=1 and t.meeting_id=?`;
