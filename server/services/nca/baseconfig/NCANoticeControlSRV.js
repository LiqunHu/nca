...
const logger = require('../../../util/Logger').createLogger('NCANoticeControlSRV');
...
const tb_notice_org = model.nca_notice_org;
...
//公告管理->公告列表
exports.NCANoticeControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr='select t.notice_id,t.notice_title,t.notice_state,t.created_at,ut.username,t.notice_detail,t.notice_question,t.notice_answer,t.notice_refuse_remark from tbl_nca_notice t ' +
    ...
    try {
        ...
        if (taskallotuser) {
            ...
            for(let org of doc.orgs){
                ...
                // let sqlStr='insert into nca_notice_user (notice_id,domain_id,user_id) ' +
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr='select t.*,ut.username from tbl_nca_notice t ' +
        ...
        let queryStr1='select t.*,gt.usergroup_name from tbl_nca_notice_org t ' +
...
async function searchReceiveAct(req, res) {
    try {
        ...
        let queryStr='select t.*,st.username,ut.read_state from tbl_nca_notice_user ut ' +
            'inner join tbl_nca_notice t on ut.notice_id = t.notice_id ' +
