...
const tb_producemateriel = model.nca_producemateriel;
const tb_common_apidomain = model.common_apidomain;
const tb_uploadfile = model.nca_uploadfile;
const tb_acceptance = model.nca_produceacceptance;
const tb_process = model.nca_process;

exports.NCAProduceControlResource = (req, res) => {
...
async function searchProduce(req,res){
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = 'select m.*,p.produce_id,p.domain_id as prodoceDomainId  from tbl_nca_produce p,tbl_nca_materiel m ' +
    ...
    try {
        ...
        let queryStr =
            `select m.*, p.domain_id, p.produce_id,p.domain_id as prodoceDomainId
                from tbl_nca_produce p
                left join tbl_nca_materiel m
...
async function searchProcess(req,res){
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = 'select * from tbl_nca_produceprocess where state=1 and produce_id=?' ;
        replacements.push(doc.produce_id);
        if(doc.search_textP){
...
async function searchProcessMateriel(req,res){
    try {
        ...
        let queryStr = `select m.*,pm.produceprocess_id,pm.producemateriel_id    
            from tbl_nca_producemateriel pm 
            left join tbl_nca_materiel m on (pm.materiel_id=m.materiel_id and m.state=1 )
            left join tbl_nca_produceprocess pp on (pm.produceprocess_id=pp.produceprocess_id and pp.state=1) 
...
async function searchAcceptanceAct(req, res){
    try {
        ...
        let queryStr = `select pa.*,pp.process_name 
            from tbl_nca_produceacceptance pa 
            left join tbl_nca_produceprocess pp on (pa.produceprocess_id=pp.produceprocess_id and pp.state=1) 
