...
const logger = require('../../../util/Logger').createLogger('NCATransReceptionSRV');
const model = require('../../../model');
const FDomain = require('../../../bl/common/FunctionDomainBL');
const task = require('./NCATaskListControlSRV');

//tables
const sequelize = model.sequelize;
const tb_trafficreceptionexpense = model.nca_trafficreceptionexpense;
const tb_trafficreceptionexpensedetail = model.nca_trafficreceptionexpensedetail;
...
exports.NCATransReceptionExpenseDetailResource = (req, res) => {
...
let initAct = async (req, res) => {
    try {
        ...
        let queryStr = 'select a.*,c.name as tr_expense_confirm_name from tbl_nca_trafficreceptionexpense a ' +
        ...
        logger.error('NCATransReceptionExpenseDetailResource-searchAct-error:' + error);
...
let searchAct = async (req, res) => {
    try {
        ...
        let queryStr = 'select * from tbl_nca_trafficreceptionexpensedetail' +
...
let modifyAct = async (req, res) => {
    try {
        ...
        logger.error('NCATransReceptionExpenseDetailResource-modifyAct-error:' + error);
        common.sendFault(res, error);
    }
}
...
let uploadFileAct = async (req, res) => {
    try {
        ...
        logger.error('NCATransReceptionExpenseDetailResource-addFileAct-error:' + error);
        common.sendFault(res, error);
    }
}
...
async function setTask(req,res){
    try{// user, taskName, taskType  4, taskPerformer, taskReviewCode, taskDescription
        ...
        if (!taskallotuser) {
            ...
            from tbl_nca_trafficreceptionexpensedetail where state=1 and tr_expense_list_code = ?`;
