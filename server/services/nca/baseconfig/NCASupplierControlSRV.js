...
const logger = require('../../../util/Logger').createLogger('NCASupplierControlSRV');
...
exports.NCASupplierControlResource = (req, res) => {
    ...
    try {
        ...
        let defSupplier =
            `select * from tbl_common_domain a
             left join tbl_nca_supplier b on a.domain_id = b.domain_id
             where a.domain_type = ?`;
        ...
        if(doc.search_text){
            ...
            materiel_sale_offer = await NCABusinessCustomerControlSRV.searchPrice(req,defVal.data[0].domain_id,user.domain_id,searchParams);
        }else{
            materiel_sale_offer = await NCABusinessCustomerControlSRV.searchPrice(req,defVal.data[0].domain_id,user.domain_id,{});
        }
...
async function getUserForSupplier(req, res) {
    try {
        ...
        let queryStr = `select
             ...
             left join tbl_nca_thirdsignuser b
    ...
    try {
        ...
        let menuResult = await tb_domainmenu.findAll({
            where: {
                ...
                api_function: {
                    $in: [
                        'NCAPURCHASELISTCONTROL',
                        'NCAPURCHASEDETAILCONTROL',
                        'USERSETTING',
                        'RESETPASSWORD']
                }
            }
        ...
        if (supplier) {
            ...
            let childResult = await tb_domainmenu.findAll({
                where: {
                    ...
                    api_function: {
                        $in: ['NCAPURCHASELISTCONTROL', 'NCAPURCHASEDETAILCONTROL']
                    }
                }
            });
