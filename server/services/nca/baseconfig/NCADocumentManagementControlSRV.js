...
const logger = require('../../../util/Logger').createLogger('NCADocumentManagementControlSRV');
const model = require('../../../model');
const task = require('../baseconfig/NCATaskListControlSRV');
const moment = require('moment');

const sequelize = model.sequelize;
const tb_document = model.nca_document;
const tb_docdetail = model.nca_docdetail;
const tb_docdetailuser = model.nca_docdetailuser;
const tb_docdetailquestion = model.nca_docdetailquestion;
const tb_user = model.common_user;
const tb_docusergroup = model.nca_docusergroup;
const tb_docuser = model.nca_docuser;
...
const tb_docdetailsubmitquestion = model.nca_docdetailsubmitquestion;

//行政办公管理->文控管理接口
exports.NCADocumentManagementControlResource = (req, res) => {
...
async function searchDocAct(req, res) {
    try {
        ...
        let queryStr='select t.*,ut2.`name` as document_checker_name from tbl_nca_document t ' +
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr='select t.* from tbl_nca_docdetail t ' +
            ...
            if (modDocDetail) {
                ...
                if(docDetail && (doc.old.clause_no != doc.new.clause_no)){
                    ...
                    if(doc.new.user_ids && doc.old.user_ids != doc.new.user_ids){
                        ...
                        let queryStr = 'delete from tbl_nca_docdetailuser where docdetail_id=?';
...
async function searchQuestionAct(req, res) {
    try {
        ...
        let queryStr='select t.* from tbl_nca_docdetailquestion t ' +
            'where t.state=1 ';
        if(doc.docdetail_id){
...
async function searchUserGroupAct(req, res) {
    try {
        ...
        let queryStr='select t.docusergroup_id,ug1.usergroup_name as p_usergroup_name,ug2.usergroup_name as usergroup_name from tbl_nca_docusergroup t ' +
    ...
    try {
        ...
        if(modDoc.document_state==0||modDoc.document_state==2){
            if (modUG) {
                ...
                let queryStr = 'delete from tbl_nca_docuser where usergroup_id=? and document_id=?';
...
async function modifyDocumentState(applyState,description,document_id,applyApprover){
    ...
    if(applyState==3){
        ...
        if(document.document_type==1){
            //内部文件
            //查询内部文件责任人
            let queryStr = 'select t.*,ut.usergroup_id from tbl_nca_docdetailuser t left join tbl_common_user ut on t.user_id= ut.user_id ' +
                'where t.state=1 and t.document_id= ?';
            ...
            //外部文件
            let queryStr = 'select t.* from tbl_nca_docuser t where t.state=1 and t.document_id= ?';
            let replacements =[document_id];
            userArr = await sequelize.query(queryStr, {
                replacements: replacements,
                type: sequelize.QueryTypes.SELECT,
...
async function searchFilesAct(req, res) {
    try {
        ...
        let queryStr = `select * from tbl_nca_document t where t.state=1 and t.document_id=?`;
...
async function searchTaskDetailAct(req, res) {
    try {
        ...
        replacements.push(doc.document_id);
        let queryStr='select t.*,ut.read_state from tbl_nca_docdetail t ' +
            'inner join tbl_nca_docuser ut on (t.docdetail_id = ut.docdetail_id and ut.user_id=?)' +
            'where t.state=1 and t.document_id=?';
