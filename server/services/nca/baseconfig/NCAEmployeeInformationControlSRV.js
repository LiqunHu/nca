...
const logger = require('../../../util/Logger').createLogger('NCAEmployeeInformationControlSRV');
...
const tb_user_contract = model.nca_customercontract;
const tb_user_work_experience = model.nca_customerworkexperience;
const tb_uploadfile = model.nca_uploadfile;
const tb_custorgstructure = model.nca_custorgstructure;
const tb_department = model.nca_department;
const tb_position = model.nca_position;
const tb_operator = model.nca_operator;

//人力资源管理->员工信息列表
exports.NCAEmployeeInformationControlResource = (req, res) => {
...
async function initAct(req, res) {
    ...
    try{
        ...
        let queryStr = 'select t.department_id as id,t.department_name as text from tbl_nca_department t ' +
        ...
        let queryStr3 = 'select b.reimburserank_id as id,b.reimburserank_name as text from tbl_common_domain a left join tbl_nca_reimburserank b on a.domain_id = b.domain_id' +
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr = 'select t.*,ct.user_form,ct.entry_date,ct.departure_date,ct.departure_reason,' +
            ...
            'left join tbl_nca_customer ct on t.user_id = ct.user_id ' +
            ...
            'left join tbl_nca_custorgstructure ot on t.user_id = ot.user_id and ot.state=1 ' +
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr1 = 'select * from tbl_nca_customercontract t where t.user_id=? ' +
        ...
        let queryStr2 = 'select * from tbl_nca_customerworkexperience t ' +
...
async function searchFilesAct(req, res) {
    try {
        ...
        let queryStr = `select t.*,ut.username from tbl_nca_customercontract t 
...
async function searchDetailImgAct(req, res) {
    try {
        ...
        let queryStr = `select * from tbl_common_user t left join tbl_nca_customer ct on t.user_id=ct.user_id where ct.photo_title='生活照' and t.user_id=?`;
