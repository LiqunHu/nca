...
const logger = require('../../../util/Logger').createLogger('NCASmallProgramSRV');
...
let searchAct = async(req, res)=> {
    try {
        ...
        let user = req.user
        let returnData = {};
        let queryStr = `select d.*, u.file_url from tbl_nca_decorateinfo d
        left join tbl_nca_uploadfile u on d.decorate_img_id = u.file_id
         where d.state = ? `;
