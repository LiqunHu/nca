...
const logger = require('../../../util/Logger').createLogger('NCAMeetingRoomManageControlSRV');
...
//行政办公管理->会议室数据列表
exports.NCAMeetingRoomManageControlResource = (req, res) => {
...
async function searchRoomAct(req, res) {
    try {
        ...
        let queryStr='select t.* , ut.name as meetinguser_name,ut2.name as equipmentuser_name from tbl_nca_meetingroom t ' +
...
async function searchEquipmentAct(req, res) {
    try {
        ...
        let queryStr='select t.* from tbl_nca_meetingroomequipment t ' +
...
async function getMeetingAct(req,res){
    try {
        ...
        if(doc.assets_type==1){
            //固定资产
            queryStr=`select * from tbl_nca_fixedassetscheckdetail t,tbl_nca_fixedassetscheck tt 
            ...
            //低值易耗品
            queryStr=`select * from tbl_nca_consumablesdetail c 
