...
const tb_productplandetail = model.nca_productplandetail;
const tb_productplanrelated = model.nca_productplanrelated;
const tb_productplanprocedure = model.nca_productplanprocedure;
const tb_productmaterielverify = model.nca_productmaterielverify;
const tb_task = model.nca_task;

const TaskListControlSRV = require('../baseconfig/NCATaskListControlSRV');

exports.NCAProductPlanControlResource = (req, res) => {
    ...
    try {
        ...
        let queryStr =
            `select t.department_id as id, t.department_name as text
             from tbl_nca_department t
             left join tbl_nca_department pt
    ...
    try {
        let queryStr =
            `select pp.product_id, pp.domain_id, pp.design_number, pp.valid_state, pp.order_id, pp.workshop_id, mt.*
             from tbl_nca_productplan pp
             left join tbl_nca_materiel mt
    ...
    try {
        let queryStr =
            `select
             ...
             , mt.*
             from tbl_nca_productplandetail ppd
             left join tbl_nca_materiel mt
    ...
    try {
        let queryStr =
            `select ppr.product_rlt_id, ppr.rlt_materiel_code, ppr.prd_type, ppr.prd_number, mt.*
             from tbl_nca_productplanrelated ppr
             left join tbl_nca_materiel mt
    ...
    try {
        let queryStr =
            `select ppp.product_procedure_id, ppp.rlt_materiel_code, ppp.priority
             ...
             from tbl_nca_productplanprocedure ppp
             left join tbl_nca_productionprocedure pp
...
async function getMaterielInfo(req, domainId) {
    const replacements = [];
    const api_name = 'NCAMATERIELCONTROL';
    const dlist = [];
...
async function getMateriel(req,res){
    try {
        ...
        let user = req.user,api_name = 'NCAMATERIELCONTROL',dlist = [];
        ...
        if(doc.tableName === 'productDetail' && doc.materiel_id){
            // queryStr+=' and materiel_id<>(select materiel_id from tbl_nca_produce where produce_id=?)'
...
async function searchPlanVerify(req, res) {
    ...
    try {
        const queryStr =
            `select
             orq.*
             from tbl_nca_orderrequire orq
             left join tbl_nca_productmaterielverify pmv
    ...
    try {
        let queryStr =
            `select
             ...
             , mat.*
             from tbl_nca_productmaterielverify pmv
             left join tbl_nca_productplandetail ppd
             on pmv.product_ppd_id = ppd.product_dtl_id
             left join tbl_nca_materiel mat
    ...
    try {
        let queryStr =
            `select
             ...
             , mat.materiel_id, mat.materiel_code, mat.materiel_name, mat.materiel_format, mat.materiel_unit
             from tbl_nca_productmaterielverify pmv
             left join tbl_nca_productplanrelated ppd
    ...
    try {
        let queryStr =
            `select
             ...
             , ppc.*
             from tbl_nca_productmaterielverify pmv
             left join tbl_nca_productplanprocedure ppd
             on pmv.product_ppd_id = ppd.product_procedure_id
             left join tbl_nca_productionprocedure ppc
...
async function getVerifyListFromProductPlan(req, product_id, verifyType) {
    const queryStr =
        `select
         orq.require_user_id
         from tbl_nca_productmaterielverify pmv
         left join tbl_nca_productplan pp
         on pmv.product_plan_id = pp.product_id
         left join tbl_nca_orderrequire orq
...
async function getVerifyListFromProductPlanRelated(req, product_id, verifyType) {
    const queryStr =
        `select
         orq.require_user_id
         from tbl_nca_productmaterielverify pmv
         left join tbl_nca_productplanrelated ppr
...
async function getVerifyListFromProductPlanProcedure(req, product_id) {
    const queryStr =
        `select
         orq.require_user_id
         from tbl_nca_productmaterielverify pmv
         left join tbl_nca_productplanprocedure ppc
