...
const logger = require('../../../util/Logger').createLogger('NCADocumentNoticeControlSRV');
...
//行政办公管理->文件通知接口
exports.NCADocumentNoticeControlResource = (req, res) => {
...
async function searchDocAct(req, res) {
    try {
        ...
        let queryStr='select t.*,st.read_state from tbl_nca_document t ' +
            'inner join tbl_nca_docuserstate st on t.document_id = st.document_id ' +
...
async function submitNBAct(req, res) {
    try {
        let doc = common.docTrim(req.body);
        let user = req.user;
        let queryStr1='select t.* from tbl_nca_docdetail t ' +
            'inner join tbl_nca_docuser ut on (t.docdetail_id = ut.docdetail_id and ut.user_id=?)' +
...
async function searchQuestionAct(req, res) {
    try {
        ...
        let queryStr='select t.docdetailquestion_id,t.document_id,t.docdetail_id,t.question_title,t.question_a,' +
            't.question_b,t.question_c,t.question_d,t.question_answer,ut.submit_question_answer from tbl_nca_docdetailquestion t ' +
            'inner join tbl_nca_docdetailsubmitquestion ut on (t.docdetailquestion_id = ut.docdetailquestion_id and ut.user_id=?)' +
            'where t.state=1 ';
        let replacements =[user.user_id];
        if(doc.docdetail_id){
    ...
    try {
        ...
        for (let r of result) {
            ...
            let ufs = await tb_uploadfile.findAll({
                where: {
                    api_name: 'NCADOCUMENTMANAGEMENTCONTROL',
                    srv_id: document,
                    state: GLBConfig.ENABLE
                }
            })
