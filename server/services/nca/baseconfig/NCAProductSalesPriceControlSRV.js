...
const logger = require('../../../util/Logger').createLogger('NCAProductSalesPriceControlSRV');
...
exports.NCAProductSalesPriceControlResource = (req, res) => {
...
async function searchTemplateAct(req, res) {
    try {
        ...
        let queryStr = 'select * from tbl_nca_producepricetemplate t where t.state=? and t.domain_id=?';
        if (doc.search_text) {
...
async function modifyTemplateDetailAct(req, res) {
    try {
        ...
        let queryStr = 'delete from tbl_nca_producepricetemplatedetail where domain_id=? and materiel_id=? and producepricetemplate_id=? ';
