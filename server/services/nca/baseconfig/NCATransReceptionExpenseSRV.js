...
exports.NCATransReceptionExpenseResource = ( req,res ) => {
...
let searchAct = async (req,res)=>{
    try{
        ...
        let queryStr='select b.*,p.paymentconfirm_state from  tbl_common_domain a ' +
            'left join tbl_nca_trafficreceptionexpense b on a.domain_id = b.domain_id ' +
            'left join tbl_nca_paymentconfirm p on (b.tr_expense_code = p.paymentconfirm_source_code and p.state=1)' +
        ...
        logger.error('NCATransReceptionExpenseResource-searchAct:'+error);
        common.sendFault(res,error);
    }
};
//移除交通接待报销信息
let removeAct = async(req,res)=>{
    try{
        ...
        logger.error('NCATransReceptionExpenseResource-removeAct:'+error);
        common.sendFault(res,error);
    }
};