...
const NCATaskListControl = require('./NCATaskListControlSRV')
...
exports.NCAMaterielControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        // let queryStr = 'select * from tbl_nca_materiel where state = 1 and domain_id' + await FDomain.getDomainListStr(req);
...
async function addAct(req, res) {
    try {
        ...
        NCATaskListControl.createTask(user, '物料审核', '7', '',doc.review_materiel_code, '物料审核','',groupID)
...
async function modifyAct(req, res) {
    try {
        ...
        if (reviewmateriel) {
            ...
            await materiel.save()
            let groupID = common.getUUIDByTime(30);
            NCATaskListControl.createTask(user, '物料审核', '7', '',
            ...
            await materiel.save()
            let groupID = common.getUUIDByTime(30);
            NCATaskListControl.createTask(user, '物料审核', '7', '', reviewmateriel.review_materiel_code, '物料审核','',groupID)
        }
...
async function searchReviewAct(req, res) {
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = 'select * from tbl_nca_reviewmateriel where state = 1 and domain_id = ?'
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr = 'select * from tbl_nca_reviewmateriel where state = 1 and domain_id = ? and review_materiel_code = ?'
