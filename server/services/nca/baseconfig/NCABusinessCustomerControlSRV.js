...
const tb_businessdomain = model.nca_businessdomain;
const tb_produce_client = model.nca_produce_client;
const tb_producepricetemplate = model.nca_producepricetemplate;
const tb_producepricetemplatedetail = model.nca_producepricetemplatedetail;


//B端客户管理->体验店列表接口
exports.NCABusinessCustomerControlSRVResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr=`select cd.*,nb.businesscustomer_type,nb.businessregistration,nb.business_tax,nb.businessdomain_id 
                      from tbl_common_domain cd left join tbl_nca_businessdomain nb on cd.domain = nb.domain
...
async function searchSale(req, res) {
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr=`select * from tbl_nca_order where purchaser_type = 1 and state = 1`
        if (user.domain_id) {
...
async function StaffControlSRVSearch(req, res) {
    try {
        ...
        let queryStr=`select * from tbl_nca_businessdomain where state = 1`
        if (doc.domain) {
            queryStr += ' and domain = ?'
