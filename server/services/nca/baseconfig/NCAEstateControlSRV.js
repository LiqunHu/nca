...
const logger = require('../../../util/Logger').createLogger('NCAEstateControlSRV');
...
const tb_goorderroom = model.nca_goorderroom;
const tb_goconstructionnode = model.nca_goconstructionnode;
const tb_orderroom = model.nca_orderroom;
const tb_gantttasks = model.nca_gantttasks;
const tb_goordermateriel = model.nca_goordermateriel;
const tb_ordermateriel = model.nca_ordermateriel;
const tb_goacceptance = model.nca_goacceptance;
...
// 小区楼盘维护接口
exports.NCAEstateControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr = 'select tbe.*,tbd.landagent_name ' +
            'from tbl_nca_estate tbe ' +
            'left join tbl_nca_landagent tbd on (tbe.land_agent=tbd.landagent_id and tbd.state=1) ' +
...
async function searchEstate(req, res) {
    try {
        ...
        queryStr = `select e.*,l.landagent_name
         from tbl_nca_estate e
         left join tbl_nca_landagent l on (e.land_agent=l.landagent_id and l.state=1)
         where e.estate_id=?`;
        replacements.push(doc.estate_id);
        ...
        replacements = [];
        queryStr = 'select build from tbl_nca_estateroom where state=1';
        if (doc.estate_id) {
        ...
        replacements = [];
        queryStr = 'select unit from tbl_nca_estateroom where state=1';
        if (doc.estate_id) {
            queryStr += ' and estate_id=?';
            replacements.push(doc.estate_id)
        }
        ...
        replacements = [];
        queryStr = 'select roomtype_id,roomtype_name from tbl_nca_roomtype where state=1';
        if (doc.estate_id) {
            queryStr += ' and estate_id=?';
            replacements.push(doc.estate_id)
        }
...
async function queryTeam(req, res) {
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = [];
        let queryStr = 'select * from tbl_nca_estateteam a, tbl_common_user b where a.user_id = b.user_id';
        if (doc.estate_id) {
            queryStr += ' and estate_id=?';
            replacements.push(doc.estate_id)
        }
...
async function searchRoomList(req, res) {
    try {
        ...
        let queryStr = 'select * ' +
            'from tbl_nca_estateroom tbe ' +
            'left join tbl_nca_roomtype tbr on (tbe.roomtype_id=tbr.roomtype_id and tbr.state=1) ' +
            'where tbe.state=1';
        if (doc.estate_id) {
            queryStr += ' and tbe.estate_id=?';
            replacements.push(doc.estate_id)
