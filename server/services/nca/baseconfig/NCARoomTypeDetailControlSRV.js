...
const logger = require('../../../util/Logger').createLogger('NCARoomTypeDetailControlSRV');
const model = require('../../../model');

const sequelize = model.sequelize;
const tb_roomtype = model.nca_roomtype;
const tb_goacceptance = model.nca_goacceptance;//验收单的表
const tb_consNode = model.nca_goconstructionnode;//施工节点的表
const tb_uploadfile = model.nca_uploadfile;//设计文档的表
const tb_govrinfo = model.nca_govrinfo;//vr效果图表
...
exports.NCARoomTypeDetailControlResource = (req, res) => {
...
let searchAcceptanceAct = async(req, res)=> {
    try {
        ...
        // let queryStr = `select acc.* from tbl_nca_goacceptance acc left join tbl_nca_goorderroom goo on (acc.goroom_type = goo.goroom_id and goo.state=1) where acc.state=1 `;
        let queryStr = `select acc.* from 
            tbl_nca_goacceptance acc ,tbl_nca_goorderroom goo 
...
async function searchMaterilalAct(req, res){
    try {
        ...
        from tbl_nca_goordermateriel gom left join tbl_nca_materiel m on gom.gomateriel_id = m.materiel_id
...
async function searchProcessAct(req, res){
    try {
        let doc = common.docTrim(req.body), user = req.user, returnData = {},replacements=[];
        let queryStr = 'select * from tbl_nca_goconstructionnode where state=1';
        if(doc.roomtype_id){
