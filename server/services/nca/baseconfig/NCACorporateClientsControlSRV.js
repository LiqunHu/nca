...
const logger = require('../../../util/Logger').createLogger('NCACorporateClientsControlSRV');
...
const tb_produce_client = model.nca_produce_client;
const tb_producepricetemplate = model.nca_producepricetemplate;

//B端客户管理->企业客户列表接口
exports.NCACorporateClientsControlResource = (req, res) => {
...
async function initAct(req, res) {
    ...
    let queryStr = 'select producepricetemplate_id as id,producepricetemplate_name text from tbl_nca_producepricetemplate where state=1 and domain_id=?';
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr='select t.* from tbl_nca_corporateclients t ' +
    ...
    try {
        ...
        if(pricetemplate){
            ...
            let queryStr1 = 'select mt1.materiel_id ' +
                'from tbl_nca_producepricetemplatedetail t ' +
                'inner join tbl_nca_materiel mt1 on t.materiel_id = mt1.materiel_id and mt1.state=? ' +
        ...
        let queryStr = 'select b.* from (select a.* from ' +
            ...
            'from tbl_nca_produce_client pc ' +
            'inner join tbl_nca_materiel m on pc.materiel_id = m.materiel_id and m.state = ? ' +
...
async function searchCreditLineDetail(req,res){
    try {
        ...
        let queryStr='select d.*,(d.creditlinedetail_surplus_creditline+d.creditlinedetail_surplus_advance) as total_surplus_money' +
            ' from tbl_nca_creditlinedetail d where d.state = 1 and d.corporateclients_id = ?';
...
async function searchSalesAct(req, res) {
    try {
        ...
        let queryStr=`select o.* from tbl_nca_order o
