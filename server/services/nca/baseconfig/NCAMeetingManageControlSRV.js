...
const logger = require('../../../util/Logger').createLogger('NCAMeetingManageControlSRV');
const model = require('../../../model');
const moment = require('moment');

const sequelize = model.sequelize;
const tb_meeting = model.nca_meeting;
const tb_meetingequipment = model.nca_meetingequipment;
const tb_meetingattendee = model.nca_meetingattendee;
const tb_task = model.nca_task;
const tb_user = model.common_user;
const tb_usergroup = model.common_usergroup;
const tb_meetingroom = model.nca_meetingroom;
const tb_meetingroomequipment = model.nca_meetingroomequipment;

//行政办公管理->会议列表
exports.NCAMeetingManageControlResource = (req, res) => {
...
async function initAct(req, res) {
    try {
        ...
        let queryStr = 'select t.meetingroom_id as id,t.meetingroom_name as text' +
            ' from tbl_nca_meetingroom t where t.domain_id=? and state=1';
...
async function searchMeetingAct(req, res) {
    try {
        ...
        let queryStr='select t.*,rt.meetingroom_name,ut.`name` as host_name from tbl_nca_meeting t ' +
            'left join tbl_nca_meetingroom rt on t.meetingroom_id = rt.meetingroom_id ' +
...
async function addMeetingAct(req, res) {
    try {
        ...
        let replacements =[];
        let queryStr = 'select * from tbl_nca_meeting t where t.meetingroom_id=? ' +
...
async function initEquipmentAct(req, res) {
    try {
        ...
        let queryStr = 'select t.meetingroomequipment_id as id,t.equipment_name as text from tbl_nca_meetingroomequipment t where t.state=1 and t.meetingroom_id=?';
        replacements.push(doc.meetingroom_id);
...
async function searchEquipmentAct(req, res) {
    try {
        ...
        let returnData = {};
        let replacements =[];
        let queryStr='select t.*,et.equipment_name,et.equipment_unit from tbl_nca_meetingequipment t ' +
            'left join tbl_nca_meetingroomequipment et on t.meetingroomequipment_id = et.meetingroomequipment_id ' +
...
async function searchAttendeeAct(req, res) {
    try {
        ...
        let queryStr='select t.*,ut.`name`,gt1.usergroup_name as p_usergroup_name,gt2.usergroup_name as usergroup_name ' +
            'from tbl_nca_meetingattendee t ' +
...
async function searchUnitAct(req, res) {
    ...
    try{
        ...
        let queryStr = 'select t.meetingroomequipment_id as id,t.equipment_unit as text from tbl_nca_meetingroomequipment t where t.meetingroomequipment_id=?';
