...
const TaskListControlSRV = require('./NCATaskListControlSRV');
const task = require('../baseconfig/NCATaskListControlSRV');

const sequelize = model.sequelize;
const tb_project = model.nca_project;
const tb_projectdetail = model.nca_projectdetail;
const tb_projectspacedetail = model.nca_projectspacedetail;
const tb_workerprice = model.nca_workerprice;
const tb_user = model.common_user;

const tb_projectsubscribe = model.nca_projectsubscribe;//材料申购
const tb_projectconsume = model.nca_projectconsume;//材料消耗单
const tb_projectconsumedetail = model.nca_projectconsumedetail;//材料消耗单明细
const tb_projectreceive = model.nca_projectreceive;//材料收料
const tb_projectreceivedetail = model.nca_projectreceivedetail;//收料单明细

const tb_projectbudget = model.nca_projectbudget;//预算
const tb_projectcost = model.nca_projectcost;//费用
const tb_supplier = model.nca_supplier;
const tb_task = model.nca_task;
const tb_taskallot = model.nca_taskallot;
const tb_taskallotuser = model.nca_taskallotuser;

const tb_projectsubscribeorder = model.nca_projectsubscribeorder;
const tb_projectsubscribeorderdetail = model.nca_projectsubscribeorderdetail;
const tb_projectsubscribeorderdetailend = model.nca_projectsubscribeorderdetailend;

const tb_projectpurchaseorder = model.nca_projectpurchaseorder;
const tb_projectpurchaseorderdetail = model.nca_projectpurchaseorderdetail;
const tb_projectsubcribeorderdetailend = model.nca_projectsubscribeorderdetailend;

exports.NCAProjectControlResource = (req, res) => {
...
async function searchAct(req,res){
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = `select p.*, e.province, e.city, e.district, e.estate_name  from tbl_nca_project p
                left join tbl_nca_estate e on p.project_estate_id = e.estate_id
...
async function getEstate(req, res) {
    try {
        ...
        let queryStr = 'select * from tbl_nca_estate where state=1 and province=? and city=? and district=?';
...
async function addAct(req,res){
    try {
        ...
        if (!taskallotuser) {
            ...
            //添加项目详情
            let replacements = [];
            let queryStr = 'select r.*, s.goroom_id as space_id, s.goroom_name as space_name from tbl_nca_goorderroom s left join tbl_nca_roomtype r on s.roomtype_id = r.roomtype_id where r.estate_id = ?';
...
async function searchDetailAct(req,res){
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = 'select * from tbl_nca_projectdetail where state = 1 and project_id = ?'
        replacements.push(doc.project_id);
        if(doc.search_text){
...
async function searchSpaceDetailAct(req,res){
    try {
        let doc = common.docTrim(req.body),
            ...
            replacements = [],
            returnData = {};
        let queryStr = `select s.*, w.worker_cost, w.worker_unit, a.purchaseapply_id, p.apply_state from tbl_nca_projectspacedetail s
         left join tbl_nca_workerprice w on (s.worker_id =  w.worker_id)
         left join (select distinct purchaseapply_id, project_space_id from tbl_nca_purchaseapplydetail) a on (s.project_space_id = a.project_space_id)
         left join tbl_nca_purchaseapply p on (a.purchaseapply_id = p.purchaseapply_id)
...
async function getSumBudgetMoney(req,res){
    ...
    let queryStr = `select ROUND(sum(clearing_reality_money),2) as total_clearing_reality_money , 
        ...
        from tbl_nca_projectbudget where state=1 and project_id=?`;
    replacements.push(doc.project_id);
    if(doc.budget_state){
...
async function modifySubscribeState(applyState,description,projectsubscribeorder_id,applyApprover){
    try{
        if(applyState==2){//通过insert tbl_nca_projectsubscribe
...
async function searchSubcribeOrderDetailEnd(req,res){
    try {
        ...
        let queryStr = `select *,0 as purchaseorderdetail_price,0 as purchaseorderdetail_number
            from tbl_nca_projectsubscribeorderdetailend 
...
async function searchSubscribe(req,res){
    try {
        ...
        let queryStr = `select s.*,p.supplier_name,subscribe_number*subscribe_price as subscribe_money,
            ...
            from tbl_nca_projectsubscribe s 
            left join tbl_nca_supplier p on (s.supplier_id = p.supplier_id and p.state = 1)
...
async function modifySubscribeConsume(req,res){
    try {
        ...
        let queryStr = `select sum(receivedetail_number * receivedetail_price) / sum(receivedetail_number) as consume_price 
                    from tbl_nca_projectreceivedetail d,tbl_nca_projectreceive r 
...
async function getSumCostMoney(req,res){
    ...
    let queryStr = `select sum(projectcost_invoice_money + projectcost_noinvoice_money) as total_cost_money 
        from tbl_nca_projectcost where state=1 and project_id=?`;
    replacements.push(doc.project_id);
    if (doc.search_text) {
...
async function searchCost(req,res){
    try {
        ...
        let queryStr = `select * from tbl_nca_projectcost where state=1 and project_id=?`;
        replacements.push(doc.project_id);
        if (doc.search_text) {
            queryStr += ` and projectcost_name like ?  `;
            let search_text = `%${doc.search_text}%`;
...
async function searchCheck(req,res){
    try {
        ...
        queryStr = `select sum(clearing_last_reality_money) as total_clearing_last_reality_money 
            from tbl_nca_projectbudget where state=1 and budget_state = 2 and project_id = ?`;
        ...
        queryStr = `select sum(consumedetail_number*consumedetail_price) as total_consume_money 
            from tbl_nca_projectconsumedetail d,tbl_nca_projectconsume c
        ...
        queryStr=`select sum(projectcost_money) as total_projectcost_money from tbl_nca_projectcost where state=1 and project_id = ?`;
        result = await sequelize.query(queryStr, {replacements: replacements, type: sequelize.QueryTypes.SELECT});
        if(result && result.length>0){
        ...
        queryStr=`select project_check_state from tbl_nca_project where state=1 and project_id=?`;
        result = await sequelize.query(queryStr, {replacements: replacements, type: sequelize.QueryTypes.SELECT});
        if(result && result.length>0){
        ...
        queryStr=`select subscribe_name,subscribe_format,subscribe_unit,
            ...
            left join (select projectsubscribe_id, sum(receivedetail_number * receivedetail_price)/sum(receivedetail_number) as receive_price  
                from  tbl_nca_projectreceivedetail 
                where state=1 
...
async function getProjectIdByProjectsubscribeorderId(req,res){
    try {
        ...
        queryStr = `select project_id from tbl_nca_projectsubscribeorder
