...
const logger = require('../../../util/Logger').createLogger('NCAAskForLeaveControlSRV');
...
const tb_askforleave = model.nca_askforleave;
...
//请假列表管理接口
exports.NCAAskForLeaveControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr='select t.*,ut.`name` as askforuser_name from tbl_nca_askforleave t ' +
...
async function searchFilesAct(req, res) {
    try {
        ...
        let queryStr = `select * from tbl_nca_askforleave t where t.state=1 and t.askforleave_id=?`;
        ...
        for (let r of result) {
            ...
            let ufs = await tb_uploadfile.findAll({
                where: {
                    api_name: 'NCAASKFORLEAVECONTROL',
                    srv_id: askforleave,
                    state: GLBConfig.ENABLE
                }
            })
