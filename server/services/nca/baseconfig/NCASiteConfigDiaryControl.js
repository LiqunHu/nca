...
const nca_sitediary = model.nca_sitediary;
const sequelize = model.sequelize;
const tb_uploadfile = model.nca_uploadfile;

exports.NCASiteConfigDiaryControl = (req, res) => {
...
async function addDiary(req, res) {
    try {
        ...
        let temyCase = await nca_sitediary.create({
...
async function searchDiary(req, res) {
    try {
        ...
        let queryStr = `select * from tbl_nca_site_diary where state = 1 `;
...
async function deleteDiary(req, res) {
    try {
        ...
        let delWh = await nca_sitediary.findOne({
            where: {
...
async function modifyDiary(req, res) {
    try {
        ...
        let modicase = await nca_sitediary.findOne({
            where: {
                diary_id: diary_id,
                state: GLBConfig.ENABLE
            }
