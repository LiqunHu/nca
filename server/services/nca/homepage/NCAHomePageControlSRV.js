...
const logger = require('../../../util/Logger').createLogger('NCAHomePageControlSRV');
...
exports.NCAHomePageControlResource = (req, res) => {
...
async function getTotalCustomer(req, res) {
    try {
        ...
        let queryStr = 'select u.*,c.* from tbl_common_user u, tbl_nca_customer c, tbl_common_domain d where u.user_id = c.user_id and' +
...
async function getActiveOrder(req, res) {
    try {
        let doc = common.docTrim(req.body), returnData = {};
        let queryStr = "select * from tbl_nca_order where order_state not in ('CANCELLED','FINISHI') and domain_id = ?";
...
async function getLatestOrder(req, res) {
    try {
        ...
        let queryStr = `select t.*,u.username from tbl_nca_task t 
...
async function getSalesAmount(req, res) {
    try {
        let doc = common.docTrim(req.body), returnData = {};
        let queryStr = "select sum(final_offer) as total_offer from tbl_nca_order where " +
...
async function getNoitceOrderAct(req, res) {
    try {
        ...
        //通知
        let queryStr = 'select t.*,st.username, u.name as customer_name from tbl_nca_message_user t ' +
            'inner join tbl_common_user st on t.user_id = st.user_id ' +
            'left join tbl_nca_order o on o.order_id = t.message_id and t.message_user_type IN (2,8) ' +
...
let searchTaskAct = async(req, res)=> {
    try {
        ...
        let queryStr = `select * from tbl_nca_task where state = 1 `;
