...
const logger = require('../../../util/Logger').createLogger('NCAPurchaseApplyDetailControlSRV');
const model = require('../../../model');
const Sequence = require('../../../util/Sequence');
const sequelize = model.sequelize;
const moment = require('moment');
const task = require('../baseconfig/NCATaskListControlSRV');
const project11 = require('../baseconfig/NCAProjectControlSRV');
const tb_order = model.nca_order;
const tb_ordermateriel = model.nca_ordermateriel;
const tb_purchaseapply = model.nca_purchaseapply;
const tb_purchaseapplydetail = model.nca_purchaseapplydetail;
...
const tb_purchasedetail = model.nca_purchasedetail;//采购单物料明细库存表
const tb_orderworkflow = model.nca_orderworkflow;

// 采购申请单明细接口
exports.NCAPurchaseApplyDetailControlResource = (req, res) => {
...
async function getPuchaseApplyDetail(req,res){
    try {
        ...
        from tbl_nca_purchaseapplydetail pd
        left join tbl_nca_materiel m on pd.materiel_id=m.materiel_id 
        where pd.state=1 `;
        if (doc.purchaseapply_id) {
...
async function getSupplerMateiel(req,res){
    try {
        ...
        let api_name = 'NCAMATERIELCONTROL',dlist = [];
...
//                         from tbl_nca_materiel m left join tbl_nca_suppliermateriel sm on m.materiel_id = sm.materiel_id
//                         left join tbl_nca_supplier s on sm.supplier_id = s.supplier_id
    ...
    try {
        ...
        for(let d of doc){
            ...
            if(m.domain_id == user.domain_id){
                ...
                queryStr=`select s.supplier_id,s.domain_id,s.supplier_proportion,m.materiel_code,sm.suppliermateriel_purchasepricetax
                        from tbl_nca_materiel m left join tbl_nca_suppliermateriel sm on m.materiel_id = sm.materiel_id
                        inner join tbl_nca_supplier s on sm.supplier_id = s.supplier_id and s.state=1
                        where m.materiel_id = ? and m.domain_id = ?
                        order by s.supplier_proportion desc`;
                replacements.push(d.materiel_id);
                replacements.push(m.domain_id);
...
async function getOrderpace(req,res){
    try {
        ...
        let queryStr = `select * from tbl_nca_orderroom where state=1 and order_id=?`;
...
async function getpuchaseApplyPrint(req,res){
    ...
    try{
        ...
        queryStr=`select m.*,pad.apply_number from tbl_nca_purchaseapplydetail pad 
        left join tbl_nca_materiel m on (pad.materiel_id=m.materiel_id and m.state=1) 
...
async function modifyPuchaseApplyState(applyState,description,purchaseApplyId,applyApprover,applyDomain_id){
    ...
    if(applyState==2){
        ...
        queryStr=`select m.domain_id
                    from tbl_nca_purchaseapplydetail tp,tbl_nca_materiel m  
        ...
        if(result && result.length>0){
            for (let r of result) {
                ...
                if(r.domain_id != NowDoMainId){
                    ...
                    queryStr=`select tp.materiel_id ,tp.apply_number,tp.project_space_id as order_ids from tbl_nca_purchaseapplydetail tp, tbl_nca_materiel m
                                where tp.state = 1
