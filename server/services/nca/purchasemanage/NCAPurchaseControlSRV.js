...
const logger = require('../../../util/Logger').createLogger('NCAPurchaseControlSRV');
...
const tb_alldemand = model.nca_alldemand;//总需求表
const tb_netdemand = model.nca_netdemand;//净需求表
const tb_order = model.nca_order;//销售单
const tb_ordermateriel = model.nca_ordermateriel;//销售单物料明细
const tb_purchaseorder = model.nca_purchaseorder;//采购单(包含申请单)
const tb_purchasedetail = model.nca_purchasedetail;//采购单物料明细
const tb_warehouse = model.nca_warehouse//仓库
const tb_stockmap = model.nca_stockmap;//库存表
const tb_purchaseapply = model.nca_purchaseapply;//采购单申请表
const tb_orderworkflow = model.nca_orderworkflow;
const tb_thirdsignuser = model.nca_thirdsignuser;

const tb_productivetask = model.nca_productivetask;// 生产任务单
const tb_productivetaskdetail = model.nca_productivetaskdetail;// 生产任务单明细（投料）
const tb_productivetaskrelated = model.nca_productivetaskrelated;// 生产任务单明细（联产品）
const tb_productplandetail = model.nca_productplandetail;// 产品规划明细
const tb_productplan = model.nca_productplan;// 产品规划
const tb_productplanrelated = model.nca_productplanrelated;// 产品规划（联产品）

const NCABusinessCustomerControlSRV = require('../baseconfig/NCABusinessCustomerControlSRV');

// mrp主接口
exports.NCAPurchaseControlResource = (req, res) => {
...
async function checkInventory(params,allDemand){
    ...
    //判断库存
    queryStr=`select s.* from tbl_nca_stockmap s,tbl_nca_warehouse w
...
async function allDemand(params){
    try{
        ...
        queryStr=`
            ...
            from tbl_nca_ordermateriel om,tbl_nca_order o
...
async function netDemand(params){
    try{
        ...
        queryStr=`select a.*,m.domain_id,m.materiel_manage,m.materiel_source,materiel_amto 
            from tbl_nca_alldemand a,tbl_nca_materiel m
...
async function production(params){
    ...
    queryStr=`select n.*,m.materiel_amto 
        from tbl_nca_netdemand n,tbl_nca_materiel m
    ...
    for(let nd of netDemand){
        ...
        //分解产品
        replacements = [];
        queryStr = `select pd.*,m.materiel_source from tbl_nca_productplandetail pd 
            left join tbl_nca_materiel m on (pd.src_materiel_id = m.materiel_id and m.state = 1) 
...
async function purchase(params){
    try{
        ...
        queryStr=`select m.domain_id from tbl_nca_netdemand n,tbl_nca_materiel m
            where n.state=1 and m.state=1 and n.materiel_id=m.materiel_id 
            ...
            for (let r of result) {
                ...
                if(r.domain_id != params.nowDoaminId){
                    ...
                    for(let sp of resultDetail){
                        ...
                        let materiel_sale_offer = await NCABusinessCustomerControlSRV.searchPrice({}, r.domain_id,params.nowDoaminId,{materiel_id:sp.materiel_id});
                    ...
                    for(let rd of resultDetail){
                        ...
                        queryStr='select s.supplier_proportion,s.supplier_id,sm.suppliermateriel_purchaseprice,sm.suppliermateriel_purchasepricetax  ' +
                            'from tbl_nca_supplier s,tbl_nca_suppliermateriel sm ' +
...
async function getAllDemand(req,res){
    try{
        ...
        let queryStr='select m.*,d.order_id,d.mrp_date,demand_amount ' +
            'from tbl_nca_alldemand d ' +
            'left join tbl_nca_materiel m on (d.materiel_id=m.materiel_id and m.state=1) ' +
...
async function getPurchaseOrder(req,res){
    try{
        ...
        let queryStr=`select p.purchaseorder_id,s.supplier_name,p.created_at,d.domain_name as order_domain,dd.domain_name as purchase_domain
            from tbl_nca_purchaseorder p
            left join tbl_nca_supplier s on (p.supplier_id=s.supplier_id and s.state=1)
