...
const logger = require('../../../util/Logger').createLogger('NCAQualityCheckControlSRV');
const model = require('../../../model');

const sequelize = model.sequelize;
const tb_user = model.user;

exports.NCAQualityCheckControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let replacements=[];
        let returnData={};
        let queryStr='select t.*,d.domain_name as order_domain,s.supplier_name,u.name as user_name from tbl_nca_qualitycheck t ' +
            'left join tbl_nca_supplier s on (t.supplier_id=s.supplier_id and s.state=1) ' +
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr='select t.*,mt.materiel_code,mt.materiel_id,mt.materiel_name,mt.materiel_format,mt.materiel_unit from tbl_nca_qualitycheckdetail t ' +
...
async function qualityCheckDetail(req, doc, user) {
    ...
    b.materiel_cost, concat(b.materiel_tax * 100, '%') as materiel_tax from tbl_nca_qualitycheckdetail a 
    left join tbl_nca_materiel b on a.materiel_id = b.materiel_id where true and a.state = 1 and a.qualitycheck_id = ?`;
    let replacements = [];
    replacements.push(doc.qualitycheck_id);
...
async function QualityCheckDetailPrintAct(req, res) {
    try {
        ...
        let fileUrl = await common.ejs2File('nca/purchasemanage/NCAQualityCheckDetail.ejs', {
            ejsData: {
