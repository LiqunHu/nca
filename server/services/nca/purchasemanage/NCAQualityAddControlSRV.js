...
const logger = require('../../../util/Logger').createLogger('NCAQualityAddControlSRV');
...
const tb_purchasedetail = model.nca_purchasedetail;
const tb_purchaseorder = model.nca_purchaseorder;
const tb_return = model.nca_return;
const tb_returndetail = model.nca_returndetail;
const tb_task = model.nca_task;
const tb_taskallot = model.nca_taskallot;
const tb_taskallotuser = model.nca_taskallotuser;
const tb_receiptitem = model.nca_receiptitem;
const tb_receipt = model.nca_receipt;


exports.NCAQualityAddControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr=`select r.*, s.supplier_name 
                      from tbl_nca_receipt r 
                      left join tbl_nca_supplier s on (r.supplier_id = s.supplier_id and s.state=1)
                      left join tbl_nca_purchaseorder p on (p.purchaseorder_id = r.purchaseorder_id) 
                      where r.domain_id = ? `;
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr=`select ri.*, ri.receipt_item_number-ri.qualified_number as check_num, 
                      ...
                      pd.order_ids 
                      from tbl_nca_receiptitem ri 
                      left join tbl_nca_materiel mt on (ri.materiel_id = mt.materiel_id and mt.state = 1)
                      left join tbl_nca_purchasedetail pd on (ri.purchasedetail_id = pd.purchasedetail_id and pd.state = 1)
                      where ri.receipt_id = ? `;
...
async function submitAct(req, res) {
    ...
    try {
        ...
        for(let i=0;i<doc.checkDetail.length;i++){
            ...
            //更新tbl_nca_receipt表品质检验状态
            let replacements=[];
            let queryStr = 'select count(1) count from tbl_nca_receiptitem ri ' +
