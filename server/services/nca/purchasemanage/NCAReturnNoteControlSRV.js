...
const logger = require('../../../util/Logger').createLogger('NCAReturnNoteControlSRV');
const model = require('../../../model');

const sequelize = model.sequelize;
const tb_user = model.user;

exports.NCAReturnNoteControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr='select t.*,d.domain_name as order_domain,s.supplier_name,u.name as user_name,qt.created_at as check_date from tbl_nca_return t ' +
            'inner join tbl_nca_qualitycheck qt on t.qualitycheck_id = qt.qualitycheck_id ' +
            'left join tbl_nca_supplier s on (qt.supplier_id=s.supplier_id and s.state=1) ' +
            'left join tbl_common_user u on (qt.user_id=u.user_id and u.state=1) ' +
            'left join tbl_nca_purchaseorder p on (p.purchaseorder_id=t.purchaseorder_id) ' +
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr='select t.*,s.purchaseorder_id,mt.materiel_code,mt.materiel_id,mt.materiel_name,mt.materiel_format,mt.materiel_unit from tbl_nca_returndetail t ' +
            'left join tbl_nca_materiel mt on t.materiel_id = mt.materiel_id ' +
            'left join tbl_nca_return s on (t.return_id=s.return_id) ' +
            'where t.return_id=?';
...
async function returnNoteDetail(req, doc, user) {
    ...
    b.materiel_cost, concat(b.materiel_tax * 100, '%') as materiel_tax from tbl_nca_returndetail a 
    left join tbl_nca_materiel b on a.materiel_id = b.materiel_id where true and a.state = 1 and a.return_id = ?`;
    let replacements = [];
    replacements.push(doc.return_id);
...
async function ReturnNoteDetailPrintAct(req, res) {
    try {
        ...
        let fileUrl = await common.ejs2File('nca/purchasemanage/NCAReturnNoteDetail.ejs', {
            ejsData: {
