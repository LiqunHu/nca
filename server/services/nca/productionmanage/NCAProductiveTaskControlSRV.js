...
const logger = require('../../../util/Logger').createLogger('NCAProductiveTaskControl');
...
// 生产任务单接口
exports.NCAProductiveTaskControlResource = (req, res) => {
...
async function getProcedure(req,res){
    try {
        ...
        let queryStr = `select pp.procedure_name,ppp.priority 
            ...
            left join tbl_nca_productionprocedure pp on (ppp.procedure_id = pp.procedure_id and pp.state=1) 
...
async function getFeeding(req,res){
    try {
        ...
        let queryStr =
            `select p.*,m.materiel_code,m.materiel_name,m.materiel_format,m.materiel_unit 
            from tbl_nca_productivetaskdetail p 
            left join tbl_nca_materiel m on (p.materiel_id = m.materiel_id and m.state=1) 
...
async function getRelated(req,res){
    try {
        ...
        let queryStr =
            `select p.*,m.materiel_code,m.materiel_name,m.materiel_format,m.materiel_unit 
            from tbl_nca_productivetaskrelated p 
            left join tbl_nca_materiel m on (p.materiel_id = m.materiel_id and m.state=1) 
