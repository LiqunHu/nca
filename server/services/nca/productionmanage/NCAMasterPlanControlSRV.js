...
const logger = require('../../../util/Logger').createLogger('NCAMasterPlanControl');
...
const tb_orderproductplan = model.nca_orderproductplan;//装修订单计划详情，按工序
const tb_order = model.nca_order;
// 生产主计划接口
exports.NCAMasterPlanControlResource = (req, res) => {
...
async function personalOrder(params){
    ...
    queryStr=`select * from tbl_nca_order 
    ...
    for(let r of result){
        ...
        replacements=[];
        queryStr=`select p.*,o.order_id,o.domain_id from tbl_nca_produceprocess p,tbl_nca_order o 
...
async function getMainPaln(req,res){
    ...
    queryStr=`select op.orderproductplan_id,pp.process_name,'场地' as construction_site,
        ...
        from tbl_nca_orderproductplan op 
        left join tbl_nca_produceprocess pp on (op.produceprocess_id=pp.produceprocess_id and pp.state=1)
        left join tbl_nca_order o on (op.order_id=o.order_id and o.state=1) 
        left join tbl_nca_produce p on (o.produce_id=p.produce_id and p.state=1) 
        left join tbl_nca_materiel m on (p.materiel_id=m.materiel_id and m.state=1) 
