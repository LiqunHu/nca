...
const logger = require('../../util/Logger').createLogger('NCARegisterControlSRV');
...
const tb_nca_customer = model.nca_customer;
const tb_nca_worker = model.nca_worker;
 ...
 */
exports.NCARegisterResource = async (req, res) => {
    try {
        ...
        if (doc.type == GLBConfig.TYPE_CUSTOMER) {
            ...
            if (addUser) {
                addCustomer = await tb_nca_customer.findOne({
                    where: {
        ...
        common.transaction(async function(t) {
            if (checkResult) {
                if (addUser) {
                    ...
                    if (doc.type == GLBConfig.TYPE_CUSTOMER) {
                        let addCustomer = await tb_nca_customer.create({
                            user_id: addUser.user_id,
                    ...
                    if (doc.type == GLBConfig.TYPE_WORKER) {
                        addWorker = await tb_nca_worker.create({
                            user_id: addUser.user_id
                        }, {
                            transaction: t
                        });
