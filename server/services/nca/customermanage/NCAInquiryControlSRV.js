...
const logger = require('../../../util/Logger').createLogger('NCAInquiryControlSRV');
...
exports.NCAInquiryControlResource = (req, res) => {
...
let searchAct = async(req, res)=> {
    try {
        ...
        let queryStr = 'select i.*,materiel_name from tbl_nca_inquiry i ' +
            'left join tbl_nca_produce p on (i.template_id = p.produce_id and p.state=1) ' +
