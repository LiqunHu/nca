...
const logger = require('../../../util/Logger').createLogger('NCACustomerControlSRV');
const model = require('../../../model');
const point = require('../baseconfig/NCAPointControlSRV');
...
const tb_customercontact = model.nca_customercontact;
...
exports.NCACustomerControlResource = (req, res) => {
...
async function initAct(req, res) {
    ...
    queryStr = 'select a.*,b.customer_level,b.customer_remarks,b.customer_state,b.customer_type,b.decorate_address ' +
        'from tbl_common_user a,tbl_nca_customer b where a.user_id = b.user_id and a.user_id=?';
    ...
    let queryStr2 = 'select o.* from tbl_common_user t ' +
        'left join tbl_nca_order o on t.user_id = o.user_id ' +
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr='select a.*,b.*,c.*,a2.`name` as salesperson_name, a2.`created_at` ' +
            ...
            'left join tbl_nca_customer b on a.user_id = b.user_id ' +
...
async function searchCustomerAct(req, res) {
    try {
        ...
        let queryStr = 'select a.*,b.`name` as operator_name from tbl_nca_customercontact a,tbl_common_user b where a.contact_operator = b.user_id'
        let replacements =[];
        if (doc.user_id){
...
let searchOrderAct = async(req, res)=> {
    try {
        ...
        tbl_nca_order o left join tbl_common_user u on o.user_id = u.user_id
...
async function searchAppointmentAct(req, res) {
    try {
        ...
        let queryStr = 'select * from tbl_nca_appointment , tbl_nca_customer where tbl_nca_customer.user_id = tbl_nca_appointment.user_id and domain_id = ?'
        ...
        if (doc.user_id) {
            queryStr += ' and tbl_nca_customer.user_id = ?'
            replacements.push(doc.user_id)
        }
...
async function searchPointhAct(req, res) {
    try {
        ...
        let queryStr = 'select a.user_id,a.username,a.name,a.phone,a.gender, ' +
            ...
            'left join tbl_nca_customer b on a.user_id = b.user_id ' +
            'left join (select * from tbl_nca_customerpoint where customerpoint_id in (select max(customerpoint_id) from tbl_nca_customerpoint group by user_id)) c on a.user_id = c.user_id ' +
