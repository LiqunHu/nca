...
const logger = require('../../../util/Logger').createLogger('NCAComplaintControlSRV');
...
const tb_complaint = model.nca_complaint;
const tb_userGroup = model.common_usergroup;
const tb_customer = model.nca_customer;
const tb_order = model.nca_order;
const tb_ownerfeedback = model.nca_ownerfeedback;

exports.NCAComplaintControlResource = async (req, res) => {
...
let searchAct = async(req, res)=> {
    try {
        ...
        let queryStr = `select * from tbl_nca_complaint where state = 1 and domain_id` + await FDomain.getDomainListStr(req);
    ...
    try {
        let queryStr =
            `select cu.name, cu.phone, nof.* 
             from tbl_nca_ownerfeedback nof 
