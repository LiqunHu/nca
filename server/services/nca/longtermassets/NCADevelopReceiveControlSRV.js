...
const logger = require('../../../util/Logger').createLogger('NCADevelopReceiveControlSRV');
...
// 研发项目收料单接口
exports.NCADevelopReceiveControlResource = (req, res) => {
...
async function searchDevelopReceive(req,res){
    try {
        ...
        let queryStr = `select r.*,a.develop_code,a.develop_name,u.name  
            from tbl_nca_developreceive r
            left join tbl_nca_develop a on (r.develop_id=a.develop_id and a.state=1)
...
async function searchDevelopReceiveDetail(req,res){
    try {
        ...
        let queryStr = `select d.*,s.subscribe_name,s.subscribe_format,s.subscribe_unit,
            ...
            p.supplier_name
            from tbl_nca_developreceivedetail d 
            left join tbl_nca_developsubscribe s on (d.developsubscribe_id=s.developsubscribe_id and s.state=1) 
            left join tbl_nca_developreceive r on (d.developreceive_id=r.developreceive_id and r.state=1)
