...
const logger = require('../../../util/Logger').createLogger('NCADevelopDetailControlSRV');
...
const tb_developsubscribe = model.nca_developsubscribe;//材料申购
const tb_developconsume = model.nca_developconsume;//材料消耗单
const tb_developconsumedetail = model.nca_developconsumedetail;//材料消耗单明细
const tb_developreceive = model.nca_developreceive;//材料收料
const tb_developreceivedetail = model.nca_developreceivedetail;//收料单明细

const tb_developbudget = model.nca_developbudget;//预算
const tb_developcost = model.nca_developcost;//费用
const tb_supplier = model.nca_supplier;
const tb_develop = model.nca_develop;
const tb_task = model.nca_task;
const tb_taskallot = model.nca_taskallot;
const tb_taskallotuser = model.nca_taskallotuser;

const tb_developsubscribeorder = model.nca_developsubscribeorder;
const tb_developsubscribeorderdetail = model.nca_developsubscribeorderdetail;
const tb_developsubscribeorderdetailend = model.nca_developsubscribeorderdetailend;

const tb_developpurchaseorder = model.nca_developpurchaseorder;
const tb_developpurchaseorderdetail = model.nca_developpurchaseorderdetail;
const tb_developsubcribeorderdetailend = model.nca_developsubscribeorderdetailend;

// 研发详情接口，包括构建预算，材料申购，采购，收料，人工结算，耗用，费用，验收8个子模块
exports.NCADevelopDetailControlResource = (req, res) => {
...
async function getDevelopInfo(req,res){
    try {
        ...
        let queryStr = `select a.*,d.department_name,
            ...
            from tbl_nca_develop a 
            left join tbl_nca_department d on (a.develop_departmant_id = d.department_id and d.state = 1) 
...
async function modifySubscribeState(applyState,description,developsubscribeorder_id,applyApprover){
    try{
        if(applyState==2){//通过insert tbl_nca_developsubscribe
...
async function searchSubcribeOrderDetailEnd(req,res){
    try {
        let doc = common.docTrim(req.body), user = req.user, returnData = {},replacements=[];
        let queryStr = `select *,0 as purchaseorderdetail_price,0 as purchaseorderdetail_number from tbl_nca_developsubscribeorderdetailend 
...
async function searchSubscribe(req,res){
        try {
            ...
            from tbl_nca_developsubscribe s 
...
async function modifySubscribeConsume(req,res){
    try {
        ...
        let queryStr = `select sum(receivedetail_number * receivedetail_price) / sum(receivedetail_number) as consume_price 
                    from tbl_nca_developreceivedetail d,tbl_nca_developreceive r 
...
async function getSumBudgetMoney(req,res){
    ...
    let queryStr = `select sum(clearing_reality_money) as total_clearing_reality_money , 
        ...
        from tbl_nca_developbudget where state=1 and develop_id=?`;
    replacements.push(doc.develop_id);
    if(doc.budget_state){
...
async function getSumCostMoney(req,res){
    ...
    let queryStr = `select sum(developcost_invoice_money + developcost_noinvoice_money) as total_cost_money 
        from tbl_nca_developcost where state=1 and develop_id=?`;
    replacements.push(doc.develop_id);
    if (doc.search_text) {
...
async function searchCost(req,res){
    try {
        ...
        let queryStr = `select * from tbl_nca_developcost where state=1 and develop_id=?`;
        replacements.push(doc.develop_id);
        if (doc.search_text) {
            queryStr += ` and developcost_name like ?  `;
            let search_text = `%${doc.search_text}%`;
...
async function searchCheck(req,res){
    try {
        ...
        queryStr = `select sum(clearing_last_reality_money) as total_clearing_last_reality_money 
            from tbl_nca_developbudget where state=1 and budget_state = 2 and develop_id = ?`;
        ...
        queryStr = `select sum(consumedetail_number*consumedetail_price) as total_consume_money 
            from tbl_nca_developconsumedetail d,tbl_nca_developconsume c
        ...
        queryStr=`select sum(developcost_money) as total_developcost_money from tbl_nca_developcost where state=1 and develop_id = ?`;
        result = await sequelize.query(queryStr, {replacements: replacements, type: sequelize.QueryTypes.SELECT});
        if(result && result.length>0){
        ...
        queryStr=`select develop_check_state from tbl_nca_develop where state=1 and develop_id=?`;
        result = await sequelize.query(queryStr, {replacements: replacements, type: sequelize.QueryTypes.SELECT});
        if(result && result.length>0){
        ...
        queryStr=`select subscribe_name,subscribe_format,subscribe_unit,
            ...
            left join (select developsubscribe_id, sum(receivedetail_number * receivedetail_price)/sum(receivedetail_number) as receive_price  
                from  tbl_nca_developreceivedetail 
                where state=1 
