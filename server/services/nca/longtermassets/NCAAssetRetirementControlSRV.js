...
async function searchScrapAct(req, res) {
    try {
        ...
        let queryStr='select t.*,ut.`name` as user_name,ut2.`name` as scrap_checker_name from tbl_nca_longassetsscrap t ' +
...
async function searchScrapDetailAct(req, res) {
    try {
        ...
        if(doc.scrap_type==1){
            //固定资产
            queryStr='select * from tbl_nca_longassetsscrapdetail t  ' +
                'left join tbl_nca_fixedassetscheckdetail cd on t.fixedasset_id = cd.fixedassetscheckdetail_id ' +
            ...
            //待摊资产
            queryStr='select * from tbl_nca_longassetsscrapdetail t  ' +
                'left join tbl_nca_amortize cd on t.fixedasset_id = cd.amortize_id ' +
                'where t.state=1 and t.longassetsscrap_id=? ';
            ...
            //低值易耗品
            queryStr='select * from tbl_nca_longassetsscrapdetail t  ' +
                'left join tbl_nca_consumablesdetail cd on t.fixedasset_id = cd.consumables_detail_id ' +
                'where t.state=1 and t.longassetsscrap_id=? ';
...
async function searchAssetAct(req, res) {
    try {
        ...
        if(doc.scrap_type==1){
            ...
            //待摊资产
            queryStr='select * from tbl_nca_amortize t  ' +
            ...
            //低值易耗品
            queryStr='select * from tbl_nca_consumablesdetail t  ' +
...
async function modifyScrapState(applyState,description,longassetsscrap_id,applyApprover){
    ...
    if(applyState==3){
        if(scrap.scrap_type==1){
            ...
            let queryStr='select * from tbl_nca_longassetsscrapdetail t  ' +
                'left join tbl_nca_fixedassetscheckdetail cd on t.fixedasset_id = cd.fixedassetscheckdetail_id ' +
                'where t.state=1 and t.longassetsscrap_id=? ';
