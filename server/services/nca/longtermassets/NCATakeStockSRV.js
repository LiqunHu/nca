...
const logger = require('../../../util/Logger').createLogger('NCAWeeklyPlanControl');
...
const tb_consumablesdetail = model.nca_consumablesdetail;//低值易耗品列表
const tb_fixedassetscheckdetail = model.nca_fixedassetscheckdetail;//固定资产验收
const tb_amortize = model.nca_amortize;//待摊资产
const tb_common_user = model.common_user;
const tb_uploadfile = model.nca_uploadfile;
const tb_task = model.nca_task;

exports.NCATakeStockResource = (req, res) => {
...
async function initData(req, res) {
    try {
        ...
        logger.error('NCAConsumablesControlResource-initData:' + err);
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr = 'select b.*,c.name from tbl_common_domain a ' +
            'left join tbl_nca_longassettakestock b on a.domain_id = b.domain_id ' +
        ...
        logger.error('NCATakeStockSRV-searchAct:' + err);
        common.sendFault(res, err);
    }
}
...
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr = 'select a.*,t.name,t.p_usergroup_id,t.user_id,pt.position_name,dt.department_name ' +
            'from tbl_nca_longassettakestockdetail a ' +
            'left join tbl_common_user t on a.take_stock_people_id = t.user_id ' +
            'left join tbl_nca_customer ct on ct.user_id = a.take_stock_people_id ' +
            'left join tbl_nca_custorgstructure ot on a.take_stock_people_id = ot.user_id and ot.state=1 ' +
            'left join tbl_nca_department dt on ot.department_id = dt.department_id and dt.state=1 ' +
            'left join tbl_nca_position pt on ot.position_id = pt.position_id and pt.state=1 ' +
...
async function addAct(req, res) {
    try {
        ...
        logger.error('NCATakeStockSRV-addAct:' + err);
        common.sendFault(res, err);
    }
}
...
async function modifyAct(req, res) {
    try {
        ...
        logger.error('NCATakeStockSRV-modifyAct' + error);
...
async function removeAct(req, res) {
    try {
        ...
        logger.error('NCATakeStockSRV-removeAct' + error);
        common.sendFault(res, err);
    }
}
...
async function searchAssetAct(req, res) {
    try {
        ...
        if (takestock) {
            if(doc.scrap_type==1){
                //固定资产
                queryStr='select * from tbl_nca_fixedassetscheckdetail t  ' +
                    ...
                    'left join tbl_nca_department dt on t.department_id = dt.department_id and dt.state=1 ' +
                ...
                //低值易耗品
                queryStr = 'select t.*,dt.department_id,dt.department_name from tbl_nca_consumablesdetail t ' +
                    'left join tbl_nca_custorgstructure ot on t.consumables_administrator_id = ot.user_id and ot.state=1 ' +
                    'left join tbl_nca_department dt on t.department_id = dt.department_id and dt.state=1 ' +
