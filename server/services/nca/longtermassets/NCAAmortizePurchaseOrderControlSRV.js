...
// 待摊资产采购单接口
exports.NCAAmortizePurchaseOrderControlResource = (req, res) => {
...
async function getAmortizePurchaseOrder(req,res){
    ...
    let queryStr = `select a.amortize_code,a.amortize_name,po.*,s.supplier_name,u1.name as creatorName 
        from tbl_nca_amortizepurchaseorder po 
        left join tbl_nca_amortize a on (a.amortize_id = po.amortize_id and a.state=1)
...
async function getAmortizePurchaseOrderDetail(req,res){
    ...
    let queryStr = `select *,
        ...
        from tbl_nca_amortizepurchaseorderdetail 
