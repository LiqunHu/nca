...
// 低值易耗品明细接口
exports.NCAConsumablesDetailControlResource = (req, res) => {
...
async function initData(req, res) {
    try {
        ...
        logger.error('NCAConsumablesDetailControlResource-initData:' + error);
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr = 'select b.*,c.name,d.department_name from tbl_nca_consumablesdetail b '+
            ...
            'left join tbl_nca_department d on b.department_id = d.department_id '+
        ...
        logger.error('NCAConsumablesDetailControlResource-searchAct:' + err);
...
async function addAct(req, res) {
    try {
        ...
        logger.error('NCAConsumablesDetailControlResource-addAct:' + err);
        common.sendFault(res, err);
    }
}
// 修改低值易耗品
async function modifyAct(req, res) {
    try {
        ...
        logger.error('NCAConsumablesDetailControlResource-modifyAct:' + err);
        common.sendFault(res, err);
    }
}
// 删除低值易耗品
let removeAct = async (req, res) => {
    try {
        ...
        logger.error('NCAConsumablesDetailControlResource-removeAct:' + error);
        common.sendFault(res, error);
    }
};
// 上传模块
let uploadAct = async (req, res) => {
    try {
        ...
        logger.error('NCAConsumablesDetailControlResource-uploadAct:' + error);
        common.sendFault(res, error)
    }
};
// 删除文件
let removeFileAct = async (req, res) => {
    try {
        ...
        logger.error('NCAConsumablesDetailControlResource-removeFileAct:' + error);
        common.sendFault(res, error);
        return
    }
}
...
async function searchPurchDetailCheckAct(req, res) {
    try {
        ...
        let replacements = [user.domain_id, GLBConfig.ENABLE];
        let queryStr = 'select b.*,c.name,d.department_name from tbl_nca_consumablesdetail b ' +
            'inner join tbl_nca_consumables tt on b.consumables_parent_code = tt.consumables_code and tt.consumables_status=3 '+
            'left join tbl_common_domain a on a.domain_id=b.domain_id ' +
            'left join tbl_common_user c on b.consumables_administrator_id=c.user_id '+
            'left join tbl_nca_department d on b.department_id = d.department_id '+
