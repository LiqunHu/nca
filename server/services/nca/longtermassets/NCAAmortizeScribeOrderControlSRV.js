...
// 待摊资产申购单列表
exports.NCAAmortizeScribeOrderControlResource = (req, res) => {
...
async function getAmortizeSubscribeOrder(req,res){
    ...
    from tbl_nca_amortizesubscribeorder so
    left join tbl_nca_amortize a on (a.amortize_id = so.amortize_id and a.state=1)
    left join tbl_nca_amortizebudget b on (b.amortizebudget_id = so.amortizebudget_id  and b.state=1)
...
async function getAmortizeSubscribeOrderDetail(req,res){
    let doc = common.docTrim(req.body), user = req.user, returnData = {},replacements=[];
    let queryStr = `select * from tbl_nca_amortizesubscribeorderdetail 
