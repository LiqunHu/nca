...
const tb_consumables = model.nca_consumables;//低值易耗品列表
const tb_consumablesdetail = model.nca_consumablesdetail;//低值易耗品详情
const tb_department = model.nca_department;//部门
const tb_uploadfile = model.nca_uploadfile;
const tb_common_user = model.common_user;

// 低值易耗品接口
exports.NCAConsumablesControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr = 'select b.*,c.department_name,e.name from tbl_common_domain a ' +
            'left join tbl_nca_consumables b on a.domain_id = b.domain_id ' +
            'left join tbl_nca_department c on b.consumables_department_id = c.department_id ' +
        ...
        logger.error('NCAConsumablesControlResource-searchAct:' + err);
        common.sendFault(res, err);
    }
}
// 增加低值易耗品
async function addAct(req, res) {
    try {
        ...
        logger.error('NCAConsumablesControlResource-addAct:' + err);
        common.sendFault(res, err);
    }
}
// 修改低值易耗品
async function modifyAct(req, res) {
    try {
        ...
        logger.error('NCAConsumablesControlResource-modifyAct' + error);
...
async function removeAct(req, res) {
    try {
        ...
        logger.error('NCAConsumablesControlResource-removeAct' + error);
        common.sendFault(res, err);
    }
}
// 查询低值易耗品明细
async function searchDetailAct(req, res) {
    try {
        ...
        let queryStr = 'select a.*,b.name,d.department_name from tbl_nca_consumablesdetail a left join ' +
            'tbl_common_user b on a.consumables_administrator_id=b.user_id ' +
            'left join tbl_nca_department d on a.department_id=d.department_id '+
        ...
        logger.error('NCAConsumablesControlResource-searchDetailAct:' + error);
        common.sendFault(res, error);
    }
}
// 修改低值易耗品明细
let modifyDetailAct = async (req, res) => {
    try {
        ...
        if (modifyData) {
            ...
            let api_name = '‌NCACONSUMABLESDETAILCONTROLSRV';
...
let uploadAct = async (req, res) => {
    try {
        ...
        logger.error('NCAConsumablesControlResource-uploadAct:' + error);
        common.sendFault(res, error)
    }
};
// 删除文件
let removeFileAct = async (req, res) => {
    try {
        ...
        logger.error('NCAConsumablesControlResource-removeFileAct:' + error);
        common.sendFault(res, error);
        return
    }
};
