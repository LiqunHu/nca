...
const logger = require('../../../util/Logger').createLogger('NCADevelopPurchaseOrderControlSRV');
...
// 研发项目采购单接口
exports.NCADevelopPurchaseOrderControlResource = (req, res) => {
...
async function getDevelopPurchaseOrder(req,res){
    ...
    let queryStr = `select a.develop_code,a.develop_name,po.*,s.supplier_name,u1.name as creatorName  
        from tbl_nca_developpurchaseorder po 
        left join tbl_nca_develop a on (a.develop_id = po.develop_id and a.state=1)
...
async function getDevelopPurchaseOrderDetail(req,res){
    ...
    let queryStr = `select *,
        ...
        from tbl_nca_developpurchaseorderdetail 
