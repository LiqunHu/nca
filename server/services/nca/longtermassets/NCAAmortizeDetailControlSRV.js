...
const logger = require('../../../util/Logger').createLogger('NCAAmortizeDetailControlSRV');
...
const tb_amortizesubscribe = model.nca_amortizesubscribe;//材料申购
const tb_amortizeconsume = model.nca_amortizeconsume;//材料消耗单
const tb_amortizeconsumedetail = model.nca_amortizeconsumedetail;//材料消耗单明细
const tb_amortizereceive = model.nca_amortizereceive;//材料收料
const tb_amortizereceivedetail = model.nca_amortizereceivedetail;//收料单明细

const tb_amortizebudget = model.nca_amortizebudget;//预算
const tb_amortizecost = model.nca_amortizecost;//费用
const tb_supplier = model.nca_supplier;
const tb_amortize = model.nca_amortize;
const tb_task = model.nca_task;
const tb_taskallot = model.nca_taskallot;
const tb_taskallotuser = model.nca_taskallotuser;

const tb_amortizesubscribeorder = model.nca_amortizesubscribeorder;
const tb_amortizesubscribeorderdetail = model.nca_amortizesubscribeorderdetail;
const tb_amortizesubscribeorderdetailend = model.nca_amortizesubscribeorderdetailend;

const tb_amortizepurchaseorder = model.nca_amortizepurchaseorder;
const tb_amortizepurchaseorderdetail = model.nca_amortizepurchaseorderdetail;
const tb_amortizesubcribeorderdetailend = model.nca_amortizesubscribeorderdetailend;

// 待摊资产详情接口，包括构建预算，材料申购，采购，收料，人工结算，耗用，费用，验收8个子模块
exports.NCAAmortizeDetailControlResource = (req, res) => {
...
async function getAmortizeInfo(req,res){
    try {
        ...
        let queryStr = `select a.*,d.department_name,
            ...
            from tbl_nca_amortize a 
            left join tbl_nca_department d on (a.amortize_departmant_id = d.department_id and d.state = 1) 
...
async function modifySubscribeState(applyState,description,amortizesubscribeorder_id,applyApprover){
    try{
        if(applyState==2){//通过insert tbl_nca_amortizesubscribe
...
from tbl_nca_amortizesubscribeorderdetailend 
...
async function searchSubscribe(req,res){
        try {
            ...
            from tbl_nca_amortizesubscribe s 
...
async function modifySubscribeConsume(req,res){
    try {
        ...
        let queryStr = `select sum(receivedetail_number * receivedetail_price) / sum(receivedetail_number) as consume_price 
                    from tbl_nca_amortizereceivedetail d,tbl_nca_amortizereceive r 
...
async function getSumBudgetMoney(req,res){
    ...
    let queryStr = `select ROUND(sum(clearing_reality_money),2) as total_clearing_reality_money , 
        ...
        from tbl_nca_amortizebudget where state=1 and amortize_id=?`;
    replacements.push(doc.amortize_id);
    if(doc.budget_state){
...
async function getSumCostMoney(req,res){
    ...
    let queryStr = `select sum(amortizecost_invoice_money + amortizecost_noinvoice_money) as total_cost_money 
        from tbl_nca_amortizecost where state=1 and amortize_id=?`;
    replacements.push(doc.amortize_id);
    if (doc.search_text) {
...
async function searchCost(req,res){
    try {
        ...
        let queryStr = `select * from tbl_nca_amortizecost where state=1 and amortize_id=?`;
        replacements.push(doc.amortize_id);
        if (doc.search_text) {
            queryStr += ` and amortizecost_name like ?  `;
            let search_text = `%${doc.search_text}%`;
...
async function searchCheck(req,res){
    try {
        ...
        queryStr = `select sum(clearing_last_reality_money) as total_clearing_last_reality_money 
            from tbl_nca_amortizebudget where state=1 and budget_state = 2 and amortize_id = ?`;
        ...
        queryStr = `select sum(consumedetail_number*consumedetail_price) as total_consume_money 
            from tbl_nca_amortizeconsumedetail d,tbl_nca_amortizeconsume c
        ...
        queryStr=`select sum(amortizecost_money) as total_amortizecost_money from tbl_nca_amortizecost where state=1 and amortize_id = ?`;
        result = await sequelize.query(queryStr, {replacements: replacements, type: sequelize.QueryTypes.SELECT});
        if(result && result.length>0){
        ...
        queryStr=`select amortize_check_state from tbl_nca_amortize where state=1 and amortize_id=?`;
        result = await sequelize.query(queryStr, {replacements: replacements, type: sequelize.QueryTypes.SELECT});
        if(result && result.length>0){
        ...
        queryStr=`select subscribe_name,subscribe_format,subscribe_unit,
            ...
            left join (select amortizesubscribe_id, sum(receivedetail_number * receivedetail_price)/sum(receivedetail_number) as receive_price  
                from  tbl_nca_amortizereceivedetail 
                where state=1 
