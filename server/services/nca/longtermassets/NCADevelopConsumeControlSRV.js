...
const logger = require('../../../util/Logger').createLogger('NCADevelopConsumeControlSRV');
...
// 研发项目材料耗用接口
exports.NCADevelopConsumeControlResource = (req, res) => {
...
async function searchDevelopConsume(req,res){
    try {
        ...
        let queryStr = `select c.*,a.develop_code,a.develop_name,u.name as consume_creator,u1.name as consume_examine
            from tbl_nca_developconsume c
            left join tbl_nca_develop a on (c.develop_id=a.develop_id and a.state=1)
...
async function searchDevelopConsumeDetail(req,res){
    try {
        ...
        let queryStr = `select d.*,s.subscribe_name,s.subscribe_format,s.subscribe_unit,d.consumedetail_number*d.consumedetail_price as consumedetail_money   
            from tbl_nca_developconsumedetail d 
            left join tbl_nca_developconsume c on (d.developconsume_id=c.developconsume_id and c.state=1) 
