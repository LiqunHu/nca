...
// 待摊资产收料单接口
exports.NCAAmortizeReceiveControlResource = (req, res) => {
...
async function searchReceive(req,res){
    try {
        ...
        let queryStr = `select r.*,a.amortize_code,a.amortize_name,u.name  
            from tbl_nca_amortizereceive r
            left join tbl_nca_amortize a on (r.amortize_id=a.amortize_id and a.state=1)
...
async function searchReceiveDetail(req,res){
    try {
        ...
        let queryStr = `select d.*,s.subscribe_name,s.subscribe_format,s.subscribe_unit,
            ...
            p.supplier_name 
            from tbl_nca_amortizereceivedetail d 
            left join tbl_nca_amortizesubscribe s on (d.amortizesubscribe_id=s.amortizesubscribe_id and s.state=1) 
            left join tbl_nca_amortizereceive r on (d.amortizereceive_id=r.amortizereceive_id and r.state=1)
            left join tbl_nca_supplier p on (d.receivesupplier_name = p.supplier_id and p.state = 1)
