...
const logger = require('../../../util/Logger').createLogger('NCAFixedAssetsControlSRV');
...
const tb_department = model.nca_department;
const tb_fixedassetspurch = model.nca_fixedassetspurch;
const tb_fixedassetspurchdetail = model.nca_fixedassetspurchdetail;
const tb_fixedassetscheck = model.nca_fixedassetscheck;
const tb_fixedassetscheckdetail = model.nca_fixedassetscheckdetail;
const tb_fixedassetsrepair = model.nca_fixedassetsrepair;
const tb_fixedassetsrepairmaterials = model.nca_fixedassetsrepairmaterials;
...
exports.NCAFixedAssetsControlResource = (req, res) => {
...
async function searchPurchAct(req, res) {
    try {
        ...
        let queryStr='select t.*,dt.department_name,ut.`name` as purch_checker_name from tbl_nca_fixedassetspurch t ' +
            'left join tbl_nca_department dt on t.department_id = dt.department_id ' +
...
async function searchPurchDetailAct(req, res) {
    try {
        ...
        let queryStr='select t.*,p.purch_state from tbl_nca_fixedassetspurchdetail t,tbl_nca_fixedassetspurch p ' +
...
async function searchCheckAct(req, res) {
    try {
        ...
        let queryStr='select t.*,ut.`name` as user_name,ut2.`name` as check_checker_name from tbl_nca_fixedassetscheck t ' +
...
async function searchCheckDetailAct(req, res) {
    try {
        ...
        let queryStr='select t.*,dt.department_name,ut.`name` as user_name from tbl_nca_fixedassetscheckdetail t ' +
            'left join tbl_common_user ut on t.user_id = ut.user_id ' +
            'left join tbl_nca_department dt on t.department_id = dt.department_id ' +
...
async function serachFixedAssets(req, res) {
    try {
        ...
        let replacements =[user.domain_id];
        let queryStr='select t.*,dt.department_name,ut.`name` as user_name from tbl_nca_fixedassetscheckdetail t ' +
            'inner join tbl_nca_fixedassetscheck tt on (t.fixedassetscheck_id = tt.fixedassetscheck_id and tt.check_state=3 and tt.state=1)' +
            'left join tbl_common_user ut on t.user_id = ut.user_id ' +
            'left join tbl_nca_department dt on t.department_id = dt.department_id ' +
...
async function searchRepairAct(req, res) {
    try {
        ...
        let returnData = {};
        let replacements =[user.domain_id];
        let queryStr='select t.*,ft.fixedassets_no,ft.fixedassets_name,ft.fixedassets_model,ft.fixedassets_unit,ut.`name` as repair_checker_name from tbl_nca_fixedassetsrepair t ' +
            'left join tbl_nca_fixedassetscheckdetail ft on t.fixedassetscheckdetail_id=ft.fixedassetscheckdetail_id ' +
...
async function searchRepairMaterialsAct(req, res) {
    try {
        ...
        let queryStr='select t.* from tbl_nca_fixedassetsrepairmaterials t ' +
            'inner join tbl_nca_fixedassetsrepair ft on t.fixedassetsrepair_id = ft.fixedassetsrepair_id ' +
...
async function searchPurchDetailCheckAct(req, res) {
    try {
        ...
        let queryStr='select t.*,pt.fixedassetspurch_no,dt.department_name,ut.`name` as user_name from tbl_nca_fixedassetspurchdetail t ' +
            'inner join tbl_nca_fixedassetspurch pt on t.fixedassetspurch_id = pt.fixedassetspurch_id and pt.purch_state=3 ' +
            'left join tbl_common_user ut on t.user_id = ut.user_id ' +
            'left join tbl_nca_department dt on t.department_id = dt.department_id ' +
