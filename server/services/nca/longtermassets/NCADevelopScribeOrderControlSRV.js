...
const logger = require('../../../util/Logger').createLogger('NCADevelopScribeOrderControlSRV');
...
// 研发项目申购单接口
exports.NCADevelopScribeOrderControlResource = (req, res) => {
...
async function getDevelopSubscribeOrder(req,res){
    ...
    from tbl_nca_developsubscribeorder so
    left join tbl_nca_develop a on (a.develop_id = so.develop_id and a.state=1)
    left join tbl_nca_developbudget b on (b.developbudget_id = so.developbudget_id  and b.state=1)
...
async function getDevelopSubscribeOrderDetail(req,res){
    let doc = common.docTrim(req.body), user = req.user, returnData = {},replacements=[];
    let queryStr = `select * from tbl_nca_developsubscribeorderdetail 
