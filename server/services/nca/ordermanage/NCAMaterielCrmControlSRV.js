...
const logger = require('../../../util/Logger').createLogger('NCAMaterielCrmControlSRV');
...
const tb_templatemateriel = model.nca_templatemateriel;
...
const NCATaskListControl = require('../baseconfig/NCATaskListControlSRV')
...
exports.initAct = async(req, res) => {
    ...
    if (doc.order_id && doc.user_id) {
        let queryStr = 'select st.staff_type from tbl_common_user ut inner join tbl_nca_staff st on ut.user_id = st.user_id and st.order_id = ? where ut.user_id = ?';
...
exports.searchAct = async(req, res) => {
    try {
        ...
        // let queryStr = `select * from tbl_nca_ordermateriel om left join tbl_nca_materiel m on om.materiel_id = m.materiel_id
...
exports.modifyAct = async(req, res) => {
    try {
        ...
        if(doc.new.change_flag == 1 && doc.new.change_state == 1 && doc.new.task_performer) {
            ...
            NCATaskListControl.createTask(user, '物料变更审核'+materiel.materiel_code, '8', doc.new.task_performer,
...
exports.searchChangeHistoryAct = async(req, res) => {
    try {
        let doc = common.docTrim(req.body);
        let returnData = [];
        let queryStr = 'select * from tbl_nca_checkmessage t where t.check_type = "CHANGE" and t.ordermateriel_id = ? order by t.created_at desc';
