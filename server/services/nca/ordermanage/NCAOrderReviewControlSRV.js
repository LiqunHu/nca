...
const logger = require('../../../util/Logger').createLogger('NCAOrderReviewControlSRV');
...
const tb_delivery = model.nca_delivery;
const tb_deliveryitem = model.nca_deliveryitem;
...
exports.NCAOrderReviewControlResource = (req, res) => {
...
let searchSaleReviewsAct = async(req, res) => {
    try {
        ...
        let queryStr = `select o.*,d.domain_name,d.domain_contact,d.domain_phone,u.username,u.phone,u.name,c.corporateclients_name,c.corporateclients_phone  
         ...
         left join tbl_nca_corporateclients c on (o.purchaser_corporateclients_id=c.corporateclients_id and c.state=1)
...
let searchDeliveryAct = async(req, res) => {
    try {
        ...
        let queryStr = `select * from tbl_nca_delivery 
                        where order_id = ? 
...
let searchDeliveryItemAct = async(req, res) => {
    try {
        ...
        let queryStr = `select di.*, d.delivery_state, m.materiel_code, m.materiel_name, m.materiel_format, m.materiel_unit 
                        from tbl_nca_deliveryitem di 
                        left join tbl_nca_materiel m on (di.materiel_id = m.materiel_id and m.state = 1) 
                        left join tbl_nca_delivery d on (di.delivery_id = d.delivery_id and d.state = 1)
