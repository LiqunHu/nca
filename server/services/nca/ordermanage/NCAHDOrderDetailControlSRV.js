...
const logger = require('../../../util/Logger').createLogger('NCAHDOrderDetailControlSRV');
...
exports.NCAHDOrderDetailControlResource = (req, res) => {
...
let initAct = async (req, res) => {
    try {
        ...
        let produce = await sequelize.query('select a.produce_id as id, a.produce_id as value, b.materiel_name as text from tbl_nca_produce a left join tbl_nca_materiel b on (a.materiel_id=b.materiel_id) where b.state = 1 and a.state = 1 and a.domain_id = ? and b.materiel_type = 10', {
...
let searchOrderAct = async (req, res) => {
    try {
        ...
        let queryRst = await sequelize.query('select * from tbl_nca_order a left join tbl_common_user b on (a.user_id=b.user_id) where a.order_id = ?', {
...
async function startOrderReviewAct(req, res) {
    try {
        ...
        for (let r of requires) {
            let result = await tb_nca_orderreview.create({
                order_id: doc.orderId,
                require_id: r.require_id,
                review_status: '1'
            });
...
async function searchOrderDesignAct(req, res) {
    try {
        ...
        let api_name = 'NCAORDERDETAILCONTROL';
        for (let r of result) {
            let row = JSON.parse(JSON.stringify(r));
            if (r.require_type == '1') {
                row.files = []
