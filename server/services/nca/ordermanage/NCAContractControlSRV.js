...
const logger = require('../../../util/Logger').createLogger('NCAContractControlSRV');
...
const tb_receivables = model.nca_receivables;
...
exports.initAct = async (req, res) => {
    ...
    let queryStr = 'select t.*,ut.`name` as contract_operator_name from tbl_nca_order t ' +
...
exports.searchAct = async (req, res) => {
    try {
        ...
        let queryStr = 'select * from tbl_nca_uploadfile t where t.api_name="NCAORDERDETAILCONTROL" and t.srv_type = ? and t.state=1 and t.order_id=?';
