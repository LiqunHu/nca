...
const logger = require('../../../util/Logger').createLogger('NCASOrderDetailControlSRV');
...
const tb_measure = model.nca_measure;
const tb_vr = model.nca_vrinfo;
...
const tb_consNode = model.nca_constructionnode;
...
const tb_salereceiveable = model.nca_salereceivables;
const tb_ordermateriel = model.nca_ordermateriel;
const tb_corporateclients = model.nca_corporateclients;
...
exports.NCASOrderDetailControlResource = (req, res) => {
...
let searchOrderAct = async (req, res) => {
    try {
        ...
        let queryRst = await sequelize.query(`
          select *,b.username from tbl_nca_order a
          left join tbl_common_user b on (a.user_id = b.user_id)
          left join tbl_nca_roomtype c on (a.roomtype_id = c.roomtype_id)
          left join tbl_nca_estate d on (a.estate_id = d.estate_id)
          where a.order_id = ?
          `, {
...
let searchSaleReceiveableAct = async (req, res) => {
    try {
        let doc = common.docTrim(req.body);
        let returnData = {};
        let queryStr = `select * from tbl_nca_salereceivables where order_id = ?`;
...
async function submitCreditLine(req,res){
    try {
        let doc = common.docTrim(req.body);
        let materielMoney = 0;
        let queryStr = `select sum(materiel_amount*sale_price) as total_sale_money from tbl_nca_ordermateriel where state = ? and order_id = ?`;
