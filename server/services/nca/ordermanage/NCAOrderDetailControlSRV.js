...
const logger = require('../../../util/Logger').createLogger('NCAOrderDetailControlSRV');
const model = require('../../../model');
const Sequence = require('../../../util/Sequence');
const StaffControlSRV = require('./NCAStaffControlSRV');
const MaterielCrmControlSRV = require('./NCAMaterielCrmControlSRV');
const WorkPlanControlSRV = require('./NCAWorkPlanControlSRV');
const AcceptancePlanControlSRV = require('./NCAAcceptancePlanControlSRV');
const ContractControlSRV = require('./NCAContractControlSRV');
const TaskListControlSRV = require('../baseconfig/NCATaskListControlSRV');
...
const tb_template = model.nca_template;
const tb_order = model.nca_order;
const tb_uploadfile = model.nca_uploadfile;
const tb_checkflow = model.nca_checkflow;
const tb_checkmessage = model.nca_checkmessage;
...
const tb_assignment = model.nca_assignment;
const tb_history = model.nca_history;
...
const tb_orderinternalreview = model.nca_orderinternalreview;
const tb_nca_orderreview = model.nca_orderreview;
const tb_estate = model.nca_estate;
const tb_estateroom = model.nca_estateroom;
const tb_produce = model.nca_produce;
const tb_zoweeprocess = model.nca_zoweeprocess;
const tb_domainsignworker = model.nca_domainsignworker;
const tb_apidomain = model.common_apidomain;
const tb_thirdsignuser = model.nca_thirdsignuser;
const tb_gantttasks = model.nca_gantttasks;
const tb_domain = model.common_domain;
const tb_orderkujiale = model.nca_orderkujiale;
const tb_receivablesrule = model.nca_receivablesrule;
const tb_message_user = model.nca_message_user;
const tb_productplan = model.nca_productplan;

exports.NCAOrderDetailControlResource = (req, res) => {
...
let initAct = async (req, res) => {
  try {
    ...
    let queryStr = 'select u.* from tbl_nca_domainsignworker d left join tbl_common_user u on d.user_id = u.user_id where d.state =1 and d.domain_id = ?';
    ...
    let produce = await sequelize.query('select a.produce_id as id, a.produce_id as value, b.materiel_name as text from tbl_nca_produce a left join tbl_nca_materiel b on (a.materiel_id=b.materiel_id) where b.state = 1 and a.state = 1 and find_in_set(a.domain_id,?) and b.materiel_type = 10', {
  ...
  try {
    ...
    let queryRst = await sequelize.query(
      `select a.*, b.*, k.*, a.order_id order_id, a.domain_id domain_id,
                a.created_at order_created_at from tbl_nca_order a 
            left join tbl_common_user b on (a.user_id=b.user_id) 
            left join tbl_nca_orderkujiale k on a.order_id = k.order_id
            where a.order_id = ?`, {
...
let getTotalCostAct = async (req, res) => {
  try {
    ...
    let queryStr = `select sum(om.materiel_amount*m.materiel_cost) as totalCost
        from tbl_nca_ordermateriel om left join tbl_nca_materiel m on om.materiel_id = m.materiel_id
...
async function searchInternalReviewAct(req, res) {
  try {
    ...
    let queryStr = `select r.*, i.*, u.username as duty_username  from tbl_nca_orderrequire r
         left join tbl_nca_orderinternalreview i on (r.require_id = i.require_id)
...
async function startOrderAct(req, res) {
  try {
    ...
    let orderReview = await tb_nca_orderreview.findAll({
      where: {
        order_id: doc.orderId,
        review_status: {
          $ne: '3'
    ...
    if (orderReview.length > 0) {
      ...
      if (result.length > 0) {//订单物料管理中有自制品类型的物料时, 判断产品规划是否已完成审核
          ...
          let count = 0;
          for (let m of result) {
              let qStr = `select pp.* from tbl_nca_productplan pp 
                                left join tbl_nca_materiel m on pp.materiel_id = m.materiel_id 
...
async function searchOrderReviewAct(req, res) {
  try {
    ...
    let queryStr = `select r.*, i.*, u.username as duty_username  from tbl_nca_orderrequire r
         left join tbl_nca_orderreview i on (r.require_id = i.require_id)
         left join tbl_common_user u on (i.duty_user_id = u.user_id)
         where i.order_id = ?`;
    let replacements = [doc.orderId];
    let result = await sequelize.query(queryStr, {
...
async function setOrderReviewDutyAct(req, res) {
  try {
    let doc = common.docTrim(req.body),
      user = req.user;
    let result = await tb_nca_orderreview.findOne({
      where: {
    ...
    if (result) {
      ...
      let queryStr = `select r.*, i.* from tbl_nca_orderrequire r
            left join tbl_nca_orderreview i on (r.require_id = i.require_id)
...
async function searchOrderDesignAct(req, res) {
  try {
    ...
    let queryStr = `select r.*, i.* from tbl_nca_orderrequire r
         left join tbl_nca_orderdesign i on (r.require_id = i.require_id)
         where i.order_id = ?`;
    let replacements = [doc.orderId];
    let result = await sequelize.query(queryStr, {
      replacements: replacements,
      ...
      if (orderRoom) {
        ...
        let queryStr =
          `select
                     ...
                     where true
                     and domain_id = ?
                     and api_name = 'NCAMATERIELCONTROL')
                     )
                     and materiel_code = ?`;
        let replacements = [];
...
async function getBuild(req, res) {
  try {
    ...
    let queryStr = 'select build from tbl_nca_estateroom where state=1 and estate_id=? group by build';
...
async function getUnit(req, res) {
  try {
    ...
    let queryStr = 'select unit from tbl_nca_estateroom where state=1 and estate_id=? and build=? group by unit';
    replacements.push(doc.estate_id);
...
async function getRoomNo(req, res) {
  try {
    ...
    let queryStr = 'select room_id,room_no from tbl_nca_estateroom where state=1 and estate_id=? and build=? and unit=?';
    replacements.push(doc.estate_id);
    replacements.push(doc.build);
...
async function getRoomType(req, res) {
  try {
    ...
    let queryStr = 'select tbr.roomtype_id,tbr.roomtype_name,tbr.roomtype_srcage ' +
      'from tbl_nca_roomtype tbr,tbl_nca_estateroom tbe ' +
...
async function getRoomTypeByEstateId(req, res) {
  try {
    ...
    let replacements = [];
    let queryStr = 'select tbr.roomtype_id,tbr.roomtype_name,tbr.roomtype_srcage ' +
      'from tbl_nca_roomtype tbr where tbr.state=1 and tbr.estate_id=?';
    replacements.push(doc.estate_id);
...
async function getForeman(req, res) {
    try {
        ...
        //let queryStr = 'select * from tbl_nca_estate where state=1 and province=? and city=? and district=?';
        let queryStr = 'select u.* from tbl_nca_domainsignworker d left join tbl_common_user u on d.user_id = u.user_id where d.state and d.domain_id = ?';
