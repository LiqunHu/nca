...
const logger = require('../../../util/Logger').createLogger('NCAWorkPlanControlSRV');
const model = require('../../../model');

const sequelize = model.sequelize;
const tb_gantttasks = model.nca_gantttasks;
const tb_user = model.common_user;
const tb_tempCons = model.nca_templateconstruction;
const tb_node = model.nca_constructionnode;
const tb_ganttlog = model.nca_ganttlog;
const tb_order = model.nca_order;
const tb_worklog = model.nca_worklog;
const tb_crew = model.nca_crew;
const tb_history = model.nca_history;
const tb_orderacceptance = model.nca_orderacceptance;
const tb_acceptance = model.nca_acceptance;
const tb_orderroom = model.nca_orderroom;
const tb_orderworkflow = model.nca_orderworkflow;
const tb_produceprocess = model.nca_produceprocess;
const tb_produceacceptance = model.nca_produceacceptance;
...
exports.initAct = async (req, res) =>{
    ...
    let queryStr = 'select * from tbl_nca_templateconstruction where state = ? and  (domain_id = (select domain_id from tbl_common_domain where domain_type = 0) or domain_id = ?)'
    ...
    let queryStr2 = 'select * from tbl_nca_order t where t.order_id=?';
...
exports.searchAct = async (req, res) => {
    try {
        ...
        let queryStr = 'select t.*,ut.`name` as leader_name,ut.phone from tbl_nca_gantttasks t left join tbl_common_user ut on t.leader_id = ut.user_id';
        let replacements = [];
        if (doc.order_id) {
...
exports.generateAct = async (req, res) => {
    ...
    await common.transaction(async function(t) {
        ...
        let queryStr = `select opp.*, pp.process_name, pp.process_level
              from tbl_nca_orderproductplan opp left join tbl_nca_produceprocess pp on opp.produceprocess_id = pp.produceprocess_id
...
exports.searchWorkLog = async (req, res) => {
    try {
        ...
        let queryStr = 'select t.*,ut.name,ut.avatar from tbl_nca_worklog t ' +
        ...
        let queryStr1 = 'select t.*,ut.name,ut.avatar from tbl_nca_worklog t ' +
            'inner join tbl_common_user ut on t.user_id = ut.user_id ' +
            'where t.order_id = ? and t.user_id = ? and t.gantttasks_id = ? and t.state = ?';
        ...
        let queryStr2 = 'select * from tbl_nca_comment t ' +
...
exports.searchModifyLogAct = async (req, res) => {
    try {
        ...
        let queryStr = 'select t.update_content,t.update_reason,t.remark,t.created_at,ut.name,tt.text from tbl_nca_ganttlog t ' +
            'left join tbl_common_user ut on t.operator_id = ut.user_id ' +
            'inner join tbl_nca_gantttasks tt on t.gantttasks_id = tt.gantttasks_id ' +
