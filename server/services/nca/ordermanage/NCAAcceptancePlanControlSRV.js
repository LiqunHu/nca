...
const logger = require('../../../util/Logger').createLogger('NCAAcceptancePlanControlSRV');
...
exports.initAct = async (req, res) => {
    try {
        ...
        //workers
        let queryStr = `select w.*,c.*,u.* from tbl_nca_crew c left join tbl_nca_worker w on c.worker_id = w.user_id
...
exports.searchAct = async (req, res) => {
    try {
        let doc = common.docTrim(req.body),
            user = req.user,
            returnData = {};
        let queryStr = `select a.*, b.text, u.name as acceptance_submit_name from tbl_nca_orderacceptance a
