...
const logger = require('../../../util/Logger').createLogger('NCAGOrderDetailControlSRV');
...
const tb_govrinfo = model.nca_govrinfo;
const tb_orderacceptance = model.nca_orderacceptance;
const tb_thirdsignuser = model.nca_thirdsignuser;

exports.NCAGOrderDetailControlResource = (req, res) => {
...
let initAct = async (req, res) => {
    try {
        ...
        let queryStrf = 'select u.* from tbl_nca_domainsignworker d left join tbl_common_user u on d.user_id = u.user_id where d.state =1 and d.domain_id = ?';
...
let searchOrderAct = async (req, res) => {
    try {
        ...
        retData.staffInfo = [];
        let staffStr = `select s.*,u.name from tbl_nca_staff s,tbl_common_user u
...
let searchDesignAct = async (req, res) => {
    try {
        let doc = common.docTrim(req.body);
        let api_name = 'NCAROOMTYPEDETAILCONTROL'; //户型详情的设计图
...
let searchAcceptanceAct = async (req, res) => {
    try {
        let doc = common.docTrim(req.body),
            user = req.user,
            returnData = {};
        let queryStr = `select o.*, u.name as acceptance_submit_name from tbl_nca_orderacceptance o LEFT JOIN tbl_common_user u on(o.acceptance_submit_id = u.user_id) where o.state = ? and o.order_id = ?`;
