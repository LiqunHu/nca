...
const logger = require('../../../util/Logger').createLogger('NCASaleOutControlSRV');
...
exports.NCASaleOutControlResource = (req, res) => {
...
//              from tbl_nca_alldemand a
//              left join tbl_nca_materiel b on a.materiel_id = b.materiel_id
//              left join tbl_nca_order c on a.order_id = c.order_id
//              left join (select ia.order_id, ia.materiel_id, sum(ia.account_operate_amount) as done_count from tbl_nca_inventoryaccount as ia
...
async function saleOutOrderHistoryDetail(req, doc, user) {
    ...
    let queryStr =
        `select
         ...
         left join tbl_nca_purchaseorder c
...
async function otherOutHistoryDetail(req, doc) {
    ...
    let queryStr = `select a.bill_code,a.domain_id,a.order_id,a.p_order_id,a.account_operate_amount,
        ...
        from tbl_nca_inventoryaccount a
        left join tbl_nca_materiel b on a.materiel_id = b.materiel_id
        left join tbl_nca_stockapply c on a.order_id = c.stockapply_id
...
async function otherOutHistoryPrintAct(req, res) {
    try {
        ...
        let fileUrl = await common.ejs2File('nca/inventorymanage/NCAOtherOutInvoice.ejs', {
            ejsData: {
...
async function getOtherOutOrder(req, res) {
    try {
        let doc = common.docTrim(req.body),
            ...
            returnData = {};
        let replacements = [];
        let queryStr = 'select * from tbl_nca_otherstockout where state = 1 and domain_id'+ await FDomain.getDomainListStr(req);
        if (doc.search_order) {
...
async function getWarehouseZone(req, res) {
    try {
        let doc = common.docTrim(req.body);
        let replacements = [];
        let queryStr = 'select * from tbl_nca_warehousezone where state=1 and warehouse_id=?';
        replacements.push(doc.warehouse_id);
...
async function checkOutNumber(req,res){
    ...
    if(doc.materiel_manage==1){
        queryStr = 'select sum(i.item_amount) as sumItemAmount ' +
            'from tbl_nca_stockitem i,tbl_nca_stockmap m ' +
        ...
        queryStr='select * from tbl_nca_stockmap where state=1 and materiel_id=?';
        replacements.push(doc.materiel_id);
        if(doc.order_id){
            queryStr +=' and order_id=?';
            replacements.push(doc.order_id);
