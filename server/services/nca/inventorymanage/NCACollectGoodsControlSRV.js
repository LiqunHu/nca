...
const logger = require('../../../util/Logger').createLogger('NCACollectGoodsControlSRV');
...
exports.NCACollectGoodsControlResource = (req, res) => {
...
async function searchCollectsAct(req, res) {
    try {
        ...
        let queryStr=`select po.purchaseorder_id, s.supplier_name, s.supplier_id, po.created_at, po.collect_state 
                      from tbl_nca_purchaseorder po
                      left join tbl_nca_supplier s on (po.supplier_id = s.supplier_id and s.state = 1)
...
async function searchCollectsDetailAct(req, res) {
    try {
        ...
        let queryStr=`select pd.purchasedetail_id, po.purchaseorder_id, po.supplier_id, pd.purchase_number, pd.collect_number,
                      ...
                      from tbl_nca_purchasedetail pd 
                      left join tbl_nca_materiel m on (pd.materiel_id = m.materiel_id and m.state = 1)
                      left join tbl_nca_purchaseorder po on (pd.purchase_id = po.purchaseorder_id)
...
async function submitCollectsAct (req, res) {
    try {
        ...
        //更改采购单收货状态
        let replacements=[];
        let queryStr = `select * from tbl_nca_purchasedetail 
