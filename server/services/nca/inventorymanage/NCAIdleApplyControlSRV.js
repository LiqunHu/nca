...
const logger = require('../../../util/Logger').createLogger('NCAStcokInApplyControlSRV');
...
const tb_suppliermateriel = model.nca_suppliermateriel;
const tb_stockapplyitem = model.nca_stockapplyitem;
const tb_idleapplyitem = model.nca_idleapplyitem;

exports.NCAIdleApplyControlResource = (req, res) => {
...
async function searchItemAct(req,res){
    try {
        ...
        let queryStr=`select ipi.*, m.*, ia.order_id, w.warehouse_name, wz.zone_name from tbl_nca_idleapplyitem ipi
                      left join tbl_nca_idleapply ia on (ipi.idleapply_id = ia.idleapply_id)
                      left join tbl_nca_materiel m on (ipi.materiel_id = m.materiel_id and m.state = 1)
                      left join tbl_nca_warehouse w on (ipi.warehouse_id = w.warehouse_id and w.state = 1)
                      left join tbl_nca_warehousezone wz on (ipi.warehouse_zone_id = wz.warehouse_zone_id and wz.state = 1)
