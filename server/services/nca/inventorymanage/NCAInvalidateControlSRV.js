...
const logger = require('../../../util/Logger').createLogger('NCASupplierMaterielControl');
...
exports.NCAInvalidateControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        ...
        let queryStr = 'select * from tbl_nca_invalidateorder where state = 1 and domain_id'+ await FDomain.getDomainListStr(req);
...
async function getInvalidateApply(req, res) {
    ...
    //let queryStr=`select * from tbl_nca_invalidateorder where state=1 `;


    //let queryStr = `select a.*,b.name from tbl_nca_invalidateorder a left join tbl_common_user b on a.user_id = b.user_id where a.state=1 `;

    let queryStr=`select a.*,
         ap.name as apply_applicant,av.name as apply_approver
         from tbl_nca_invalidateorder a
...
let searchTable = async (req, res) => {
    try {
        ...
        m.materiel_code,m.materiel_name,m.materiel_format,m.materiel_unit,m.materiel_manage from tbl_nca_stockmap s
        left join tbl_nca_warehouse w on (s.warehouse_id = w.warehouse_id and w.state=1)
        left join tbl_nca_materiel m on (s.materiel_id = m.materiel_id and m.state = 1)
        left join tbl_nca_warehousezone wz on (s.warehouse_zone_id = wz.warehouse_zone_id and wz.state = 1)
	    ...
	    from tbl_nca_stockmap s
        left join tbl_nca_materiel m on (s.materiel_id = m.materiel_id and m.state = 1)
	    left join tbl_nca_stockitem sk on(s.stockmap_id = sk.stockmap_id)
        left join tbl_nca_warehouse w on (sk.warehouse_id = w.warehouse_id and w.state=1)
        left join tbl_nca_warehousezone wz on (sk.warehouse_zone_id = wz.warehouse_zone_id and wz.state = 1)
...
let search_mTable = async (req, res) => {
    try {
        ...
        let queryStr = `select ino.*, w.warehouse_name, wz.zone_name,m.materiel_id,m.materiel_code, m.materiel_name,m.materiel_format,m.materiel_unit,m.materiel_manage
                from tbl_nca_invalidateapplyorder ino
                left join tbl_nca_materiel m on (ino.materiel_id = m.materiel_id and m.state = 1)
                left join tbl_nca_warehouse w on (ino.warehouse_id = w.warehouse_id and w.state=1)
                left join tbl_nca_warehousezone wz on (ino.warehouse_zone_id = wz.warehouse_zone_id and wz.state = 1) where ino.state = 1`
