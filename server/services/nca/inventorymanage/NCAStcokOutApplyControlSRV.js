...
const logger = require('../../../util/Logger').createLogger('NCAStcokOutApplyControlSRV');
...
const tb_stockotherapplyout = model.nca_stockotherapplyout;
const tb_inventoryaccount = model.nca_inventoryaccount;
const tb_otherstockout = model.nca_otherstockout;


exports.NCAStcokOutApplyControlResource = (req, res) => {
...
async function searchAct(req, res) {
    try {
        let doc = common.docTrim(req.body),
            ...
            returnData = {},
            replacements = [];
        let queryStr = 'select * from tbl_nca_stockoutapply where state = 1 and domain_id'+ await FDomain.getDomainListStr(req);
        if (doc.search_text) {
...
let searchTable = async (req, res) => {
    try {
        let doc = common.docTrim(req.body),
            ...
            returnData = {},
            replacements = [];
        let queryStr = 'select s.materiel_id,m.materiel_code,m.materiel_name,m.materiel_format,m.materiel_unit,m.materiel_manage from tbl_nca_stockmap s left join tbl_nca_materiel m on s.materiel_id = m.materiel_id where m.state = 1 and s.domain_id'+ await FDomain.getDomainListStr(req);
...
let search_mTable = async (req, res) => {
    try {
        ...
        let queryStr = `select ino.*,m.materiel_id,m.materiel_code, m.materiel_name,m.materiel_format,m.materiel_unit,m.materiel_manage
                from tbl_nca_stockoutapplydetail ino
                left join tbl_nca_materiel m on (ino.materiel_id = m.materiel_id and m.state = 1)
                where ino.state = 1`
        let replacements = [];
...
async function searchOutMateriel(req, res) {
    try {
        ...
        //                from tbl_nca_otherstockout oo left join tbl_nca_stockoutapplydetail sa on oo.stockoutapply_id = sa.stockoutapply_id
        //                left join tbl_nca_materiel m on sa.materiel_id = m.materiel_id
        //                where oo.state = 1`
        ...
        let queryStr = `select sa.stockoutapplydetail_id,sa.stockoutapply_id,sa.stockoutapplydetail_amount,sa.already_amount,sa.stockoutapplydetail_type,m.materiel_id,m.materiel_name,m.materiel_code,m.materiel_format,m.materiel_unit,m.materiel_manage 
                from tbl_nca_stockoutapplydetail sa left join tbl_nca_materiel m on sa.materiel_id = m.materiel_id
                where sa.state = 1`
...
async function searchOtherApplyOtherOut(req, res) {
    try {
        ...
        //             from tbl_nca_stockotherapplyout sa left join tbl_nca_materiel m on sa.materiel_id = m.materiel_id
        //             where sa.state = 1`

        let queryStr = `select sa.*,m.*,sk.already_amount
                    from tbl_nca_stockotherapplyout sa left join tbl_nca_materiel m on sa.materiel_id = m.materiel_id
                    left join tbl_nca_stockoutapplydetail sk on sa.stockoutapplydetail_id = sk.stockoutapplydetail_id
                    where sa.state = 1`
...
let StockOtherOut = async (req, res) => {
    try {
        let doc = common.docTrim(req.body),
            user = req.user;
        let queryStrApply = 'select * from tbl_nca_stockotherapplyout where state = 1 and stockoutapply_id = ?';
        ...
        let queryStr = `select sum(stockoutapplydetail_amount) as total_stockoutapplydetail_amount,
            ...
            from tbl_nca_stockoutapplydetail where stockoutapply_id=?`;
        let replacements = [];
