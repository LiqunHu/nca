...
const logger = require('../../../util/Logger').createLogger('NCAInventoryControlSRV');
...
const tb_idleapply = model.nca_idleapply;
const tb_idleapplyitem = model.nca_idleapplyitem;


exports.NCAInventoryControlResource = (req, res) => {
...
let scanSafePurchaseAct = async(req, res) => {
    try {
        ...
        let queryStr = `select * from tbl_nca_stockmap s 
                        left join tbl_nca_materiel m on (s.materiel_id = m.materiel_id and m.state = 1) 
...
let searchStcokItemAct = async(req, res) => {
  try {
      ...
      let queryStr = `select si.stockitem_id,si.item_amount,si.warehouse_id,si.warehouse_zone_id,
                      ...
                      m.materiel_unit,w.warehouse_name,wz.zone_name from tbl_nca_stockitem si 
                      left join tbl_nca_stockmap s on (s.stockmap_id = si.stockmap_id)
                      left join tbl_nca_warehouse w on (si.warehouse_id = w.warehouse_id and w.state=1)
                      left join tbl_nca_materiel m on (s.materiel_id = m.materiel_id and m.state = 1)
                      left join tbl_nca_warehousezone wz on (si.warehouse_zone_id = wz.warehouse_zone_id and wz.state = 1)
...
exports.dealWithInventoryOut = async(warehouseId, materialId, amountOut, orderId, domainId, warehouseZoneId) => {
    ...
    if (!isSafe) {
        let queryStr = `select sum(current_amount) as total from tbl_nca_stockmap 
...
exports.dealWithSafeInventoryOut = async(warehouseId, materialId, amountOut, domainId, warehouseZoneId) => {
    ...
    let queryStr = `select si.* from tbl_nca_stockitem si
                    left join tbl_nca_stockmap s on (s.stockmap_id = si.stockmap_id)
                    left join tbl_nca_materiel m on (s.materiel_id = m.materiel_id and m.state = 1)
...
let searchMaterielAct = async(req, res)=> {
    try {
        ...
        if(doc.materiel_manage == '2'){//销售订单管理
            ...
            s.current_amount from tbl_nca_stockmap s,tbl_nca_materiel m 
            ...
            if(typeof(doc.warehouse_id) == 'undefined' && typeof(doc.warehouse_zone_id) == 'undefined'){
                ...
                queryStr =`select m.materiel_code,m.materiel_name,materiel_unit,m.materiel_format,
            si.item_amount from tbl_nca_stockmap s,tbl_nca_materiel m 
            where s.materiel_id = m.materiel_id and m.state = 1 and s.domain_id = ? and s.warehouse_id = ? and s.warehouse_zone_id = ?`
                ...
                queryStr =`select m.materiel_code,m.materiel_name,materiel_unit,m.materiel_format,
            si.item_amount from tbl_nca_stockmap s,tbl_nca_materiel m,tbl_nca_stockitem si 
...
let scanIdleAct = async(req, res) => {
    try {
        ...
        let queryStr = `select sm.* from tbl_nca_stockmap sm
                        left join tbl_nca_order o on (o.order_id = sm.order_id)
