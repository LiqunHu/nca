...
const logger = require('../../../util/Logger').createLogger('NCABuyInControlSRV');
const model = require('../../../model');
const Sequence = require('../../../util/Sequence');
const sms = require('../../../util/SMSUtil.js');
const FDomain = require('../../../bl/common/FunctionDomainBL');
const inventoryControl = require('./NCAInventoryControlSRV');

const sequelize = model.sequelize;
const tb_warehouse = model.nca_warehouse;
const tb_inventoryorder = model.nca_inventoryorder;
const tb_inventoryaccount = model.nca_inventoryaccount;
const tb_user = model.common_user;
const tb_warehousezone = model.nca_warehousezone;
...
const tb_stockitem = model.nca_stockitem;
const tb_otherinventoryorder = model.nca_otherinventoryorder;
const tb_otherinventoryaccount = model.nca_otherinventoryaccount;
const tb_stockapply = model.nca_stockapply;
const tb_qualitycheckdetail = model.nca_qualitycheckdetail;
const tb_qualitycheck = model.nca_qualitycheck;
const tb_productivetask = model.nca_productivetask;
const tb_productivetaskrelated = model.nca_productivetaskrelated;

exports.NCABuyInControlResource = (req, res) => {
...
async function getZoneByWearHouse(req, res) {
    ...
    try {
        ...
        let queryStr =
            `select warehouse_zone_id as id, zone_name as text
                from tbl_nca_warehousezone where warehouse_id = ? and state = 1`;
        let replacements = [];
        replacements.push(doc.warehouse_id);
    ...
    try {
        ...
        let queryStr =
            `select
             ...
             from tbl_nca_purchaseorder a
             left join (
             select pd.purchase_id, sum(pd.purchase_number) as total_count
             from tbl_nca_purchasedetail as pd
...
async function getOtherInOrderAct(req, res) {
    try {
        ...
        let queryStr =
            ...
            from tbl_nca_otherstockorder o
            ...
            from tbl_nca_stockapplyitem s where true group by stockapply_id) e
...
async function buyInOrderHistoryDetail(req, doc, user) {
    ...
    let queryStr =
        `select
         ...
         from tbl_nca_inventoryaccount a
         left join tbl_nca_materiel b on a.materiel_id = b.materiel_id
         left join tbl_nca_purchaseorder c on (a.order_id = c.order_id and c.order_id<>'')
         where true `;
    let replacements = [];
    if (user.domain_id) {
        queryStr += ` and a.domain_id = ?`;
...
async function inventoryAOGSms(params) {
    ...
    if (result && result.length > 0) {
        if (result[0].order_status == 3) {
            ...
            from tbl_nca_inventoryaccount i,tbl_nca_order o,tbl_common_user u
...
async function getOtherInOrderMaterielAct(req, res) {
    try {
        ...
        b.materiel_code,b.materiel_name,b.materiel_manage,b.materiel_format,b.materiel_unit
            from tbl_nca_stockapplyitem s,tbl_nca_materiel b
...
async function buyInOrderHistoryPrint(req, res) {
    try {
        ...
        let fileUrl = await common.ejs2File('nca/inventorymanage/NCABuyInInvoice.ejs', {
            ejsData: {
...
async function otherInOrderHistoryDetail(req, doc, user) {
    ...
    let queryStr = `select a.bill_code,a.domain_id,a.order_id,a.p_order_id,a.account_operate_amount,
        ...
        from tbl_nca_inventoryaccount a
        left join tbl_nca_materiel b on a.materiel_id = b.materiel_id
        left join tbl_nca_stockapply c on a.p_order_id = c.stockapply_id where true and a.account_operate_type = 3 and a.bill_code = ?`;
    let replacements = [];
    replacements.push(doc.bill_code);
    if (user.domain_id) {
        queryStr += ` and a.domain_id = ?`;
...
async function OtherInOrderHistoryPrintAct(req, res) {
    try {
        ...
        let fileUrl = await common.ejs2File('nca/inventorymanage/NCAOtherInInvoice.ejs', {
            ejsData: {
                bill_code: doc.bill_code,
...
async function getQualityCheckOrderAct(req, res) {
    ...
    let queryStr = `select q.qualitycheck_id,cu.name,if(qd.finishStock_number is null, 1, if(qd.finishStock_number < qd.qualified_number, 2, 3)) as order_status,
        ...
        from tbl_nca_qualitycheck q 
        left join tbl_common_user cu on (q.user_id=cu.user_id and cu.state=1) 
        left join (select qualitycheck_id,IFNULL(sum(qualified_number),0) as qualified_number,IFNULL(sum(finishStock_number),0) as finishStock_number from tbl_nca_qualitycheckdetail where state=1 group by qualitycheck_id) qd on (q.qualitycheck_id = qd.qualitycheck_id ) 
        where q.state=1 `;
    if (user.domain_id) {
...
async function getQualityCheckOrderDetailAct(req, res) {
    try{
        ...
        let queryStr = `select qd.qualitycheckdetail_id,qd.order_id,m.materiel_id,m.materiel_code,m.materiel_name,m.materiel_format,m.materiel_unit,qd.qualified_number,qd.finishStock_number,q.supplier_id,s.supplier_name,s.supplier  
            from tbl_nca_qualitycheckdetail qd 
            left join tbl_nca_qualitycheck q on (qd.qualitycheck_id = q.qualitycheck_id and q.state=1) 
            left join tbl_nca_materiel m on (qd.materiel_id = m.materiel_id and m.state=1) 
            left join tbl_nca_supplier s on (q.supplier_id = s.supplier_id and q.state=1) 
...
async function getProductInOrderAct(req, res) {
    try {
        ...
        let queryStr = `select pt.*, u.name from tbl_nca_productivetask pt
                        left join tbl_nca_productivetaskrelated ptr on pt.productivetask_id = ptr.productivetask_id
                        left join tbl_nca_order o on pt.order_id = o.order_id 
...
async function getProductInItemsAct(req, res) {
    try {
        ...
        let queryStr = `select ptr.*, m.materiel_code, m.materiel_name, m.materiel_format, m.materiel_unit, pt.order_id 
                        from tbl_nca_productivetaskrelated ptr 
                        left join tbl_nca_materiel m on (ptr.materiel_id = m.materiel_id and m.state = 1) 
                        left join tbl_nca_productivetask pt on ptr.productivetask_id = pt.productivetask_id 
        ...
        let queryStr2 = `select pt.*, m.materiel_code, m.materiel_name, m.materiel_format, m.materiel_unit
                         from tbl_nca_productivetask pt
                         left join tbl_nca_materiel m on (pt.materiel_id = m.materiel_id and m.state = 1) 
...
async function submitProductInItemsAct(req, res) {
    try {
        ...
        if (materiels.length > 0) {
            ...
            let queryStr = '';
            if (related) {
                queryStr = `select pt.* from tbl_nca_productivetask pt
                            left join tbl_nca_productivetaskrelated ptr on pt.productivetask_id = ptr.productivetask_id
                ...
                queryStr = `select * from tbl_nca_productivetask 
