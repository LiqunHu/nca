...
const logger = require('../../../util/Logger').createLogger('NCACheckInventoryControlSRV');
...
const tb_checkinventory = model.nca_checkinventory;
const tb_checkitem = model.nca_checkitem;
const tb_materiel = model.nca_materiel;

exports.NCACheckInventoryControlResource = (req, res) => {
...
let addAct = async(req, res)=> {
    try {
        ...
        let queryStr = `select sm.materiel_id,sum(sm.current_amount) as invertory from tbl_nca_stockmap sm
         left join tbl_nca_checkinventory ci on sm.domain_id = ci.domain_id
        ...
        //safe material
        let queryStr1 = `select * from tbl_nca_stockmap sm 
                        left join tbl_nca_stockitem si on (sm.stockmap_id = si.stockmap_id) 
...
let searchAct = async(req, res)=> {
    try {
        let doc = common.docTrim(req.body), user = req.user, returnData = {};
        let queryStr = `select ci.*,w.warehouse_name from tbl_nca_checkinventory ci
         left join tbl_nca_warehouse w on (ci.warehouse_id = w.warehouse_id and w.state = 1) 
...
let searchDetailAct = async(req, res)=> {
    try {
        ...
        let queryStr = `select * from tbl_nca_checkitem ci 
        left join tbl_nca_materiel m on ci.materiel_id = m.materiel_id 
