            ...
            <div class="clearfix">
              <p class="pagination pagination-without-border" style="width:300px">
                <a href="javascript:;" class="btn btn-primary btn-block" @click="newDegsinDialog()">
                  <i class="fa fa-edit pull-right"></i> 新建方案
                </a>
              </p>
            </div>
                ...
                <li>
                  <div style="width: 240px; border-left: 1px dashed #ddd; float: right; height: 200px; display: flex; justify-content:center; align-items:Center;">
                    ...
                    <div style="width: 240px;">
                      <a class="btn btn-info btn-block" @click="getDegsin(order)">方案设计</a>
                      <a v-if="order.desid" class="btn btn-primary btn-block" id="generateData" @click="generateKJL(order)">数据处理</a>
                      <a v-if="order.listingid" class="btn btn-success btn-block" @click="kjlSync(order)">一键同步</a>
                    </div>
                  </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body" style="height: 529px; padding: 0px;">
            ...
            <div class="experience-show" style="right: 0px;position:absolute;" v-bind:class="[ isActive ? 'selectM':'selectO']">
              ...
              <button type="button" class="btn btn-success m-r-5 m-b-5" @click="getIframeSrc('2')">方案设计</button>
  ...
  methods: {
    ...
    generateKJL: async function(order) {
      ...
      try {
        ...
        $("#generateData").addClass("disabled");
        function timeOut() {
          if (wait != 0) {
            setTimeout(function() {
              if (wait != 0) {
